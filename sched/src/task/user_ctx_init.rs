...
//!
//! Lives in `sched/` alongside `task_lifecycle.rs` so neither `sched/` nor
//! `core/` has to depend on the other. Used by the exec path; fork and clone
//! seed their child from the parent's live `UserContext` instead.
