...
//!
//! `Task` destruction is deferred to the graveyard, so release happens at the
//! `exit_cleanup_mark(TASK_EXIT_CLEANUP_ACCOUNTED)` latch instead, which fires
//! exactly once however the exit is split across CPUs. Each row owns a real
//! `Charge<TaskCount>`, so only the latch that wins the take can refund it.
...
/// Reserve one task against `account`. Returned rather than recorded, so a
/// caller that fails later unwinds through the reservation's `Drop`.
pub fn reserve(account: AccountId) -> Option<Reservation<TaskCount>> {
    try_charge::<TaskCount>(account, 1).ok()
}

/// Record `reservation` as `task_id`'s charge. An occupied row means an id was
/// reused without its predecessor's latch firing; that charge is refunded.
...
/// CPUs, refunds once. Atomics only, so it is legal from the exit path with
/// interrupts off.
