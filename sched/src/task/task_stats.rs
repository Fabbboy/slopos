...
pub fn task_record_context_switch(from: Option<&Task>, to: Option<&Task>, timestamp: u64) {
    ...
    if let Some(to) = to {
        ...
        // Task identity is an address question, not a value question:
        // `TaskInner` deliberately has no `PartialEq`. A `None` `from` counts
        // as a switch.
...
pub(super) fn record_task_created() {
    with_task_manager(|mgr| {
        // num_tasks is already incremented by task allocation.
