...
//! singleton mutators (`gdt_init`, `gdt_set_ist`, `syscall_msr_init`, …);
//! calling those after `enter_scheduler` is always a bug.
//!
//! The mint-by-token methods are kind-gated, so an AP-derived
//! `BootCtx<'_, ApInit>` cannot synthesise a `BspToken<'_>`.
//!
//! `take_for_*` mint a fresh token per call rather than moving one out of a
//! slot, so a test fixture running inside a boot step can hold one while boot
//! holds its own; "one test scope at a time" is carried by `TEST_SCOPE_ACTIVE`.
...
/// BSP-init kind marker: minted by [`take_for_boot`], authorises BSP-only
/// mutators plus [`BootCtx::bsp_token`] reconstruction.
pub struct BspInit;

/// AP-init kind marker: minted by [`take_for_ap`], authorises AP-only
/// mutators.
pub struct ApInit;

/// Test-init kind marker: minted by [`take_for_test`], authorises CPU mutators
/// but **not** OSTD `register_*` hooks — tests reach those via
...
/// so it cannot be forged; `K` names the init scope. Construction is
/// `pub(crate)`: only this crate's `take_for_*` functions can mint one.
pub struct BootCtx<'brand, K: BootKind> {
    ...
    /// `take_for_boot` always stores `Some`.
...
impl<'brand> BootCtx<'brand, BspInit> {
    /// Reconstruct a borrowed `BspToken<'brand>`: the brand was minted by the
    /// originating `run_bsp_init` closure, so this is the same capability.
    /// Boot-step fns whose signature the `boot_init!` linker-section macro
    /// fixes have no room for a separate `&BspToken` parameter and reach OSTD
    /// `register_*` hooks through here.
    #[inline]
...
/// Mint the BSP boot path's `BootCtx`, inside the
/// [`slopos_ostd::sync::run_bsp_init`] HRTB closure.
#[inline]
pub fn take_for_boot<'brand>(token: &BspToken<'brand>) -> BootCtx<'brand, BspInit> {
    BootCtx::new_unchecked(Some(*token))
}

/// Consume the `BootCtx` returned by `take_for_boot`.
#[inline]
...
/// Mint an AP boot path's `BootCtx`, inside the
/// [`slopos_ostd::sync::run_ap_init`] HRTB closure.
#[inline]
