...
/// a negated errno.
pub fn ring_register_buffers(pid: u32, raw_handle: usize, iovecs: &[(u64, u32)]) -> i32 {
    if !registry::owner_is(raw_handle, pid) {
        return eno(Errno::EBADF);
    }
    match registry::with_ring(raw_handle, |ring| ring.buffers.register_fixed(pid, iovecs)) {
        Ok(Ok(())) => 0,
        Ok(Err(e)) => eno(e),
...
pub fn ring_unregister_buffers(pid: u32, raw_handle: usize) -> i32 {
    if !registry::owner_is(raw_handle, pid) {
        return eno(Errno::EBADF);
    }
...
pub fn ring_register_pbuf_ring(pid: u32, raw_handle: usize, cmd: &RegisterBufRingCmd) -> i32 {
    if !registry::owner_is(raw_handle, pid) {
        return eno(Errno::EBADF);
    }
    match registry::with_ring(raw_handle, |ring| ring.buffers.register_provided(pid, cmd)) {
        Ok(Ok(())) => 0,
        Ok(Err(e)) => eno(e),
        Err(_) => eno(Errno::EBADF),
    }
...
pub fn ring_unregister_pbuf_ring(pid: u32, raw_handle: usize, group: u16) -> i32 {
    if !registry::owner_is(raw_handle, pid) {
        return eno(Errno::EBADF);
    }
