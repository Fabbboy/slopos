...
impl ShmBuffer {
    pub fn create(size: usize) -> Result<Self, ShmError> {
        ...
        let token_raw = sys::shm_create(size as u64, 0);
        let token = NonZeroU32::new(token_raw).ok_or(ShmError::AllocationFailed)?;

        let ptr_raw = sys::shm_map(token_raw, SHM_ACCESS_RW);
        if ptr_raw == 0 || (ptr_raw as i64) < 0 {
            sys::shm_destroy(token_raw);
...
impl Drop for ShmBuffer {
    fn drop(&mut self) {
        unsafe {
            sys::shm_unmap(self.ptr.as_ptr() as u64);
        }
        sys::shm_destroy(self.token.get());
    }
}
...
impl ShmBufferRef {
    pub fn map_readonly(token: u32, size: usize) -> Result<Self, ShmError> {
        ...
        let ptr_raw = sys::shm_map(token, SHM_ACCESS_RO);
        if ptr_raw == 0 || (ptr_raw as i64) < 0 {
            return Err(ShmError::MappingFailed);
        }
...
impl CachedShmMapping {
    pub fn map_readonly(token: u32, size: usize) -> Option<Self> {
        ...
        let vaddr = sys::shm_map(token, SHM_ACCESS_RO);
