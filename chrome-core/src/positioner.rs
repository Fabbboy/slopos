...
//!
//! The parameter set is `xdg_positioner`'s — anchor rectangle, anchor edge,
//! gravity, size, offset, work area, constraint policy — so wiring up the
//! protocol's popup role reuses this rather than growing a second placer that
//! disagrees at the edges. Geometry only: no framebuffer, no descriptors.
...
impl Rect {
    ...
    /// Whether `(x, y)` falls inside. Also the light-dismiss predicate, so the
    /// renderer's idea of the surface's extent and the input path's cannot
    /// diverge.
    #[inline]
...
/// Which way the surface extends from the anchor point: [`Gravity::BottomLeft`]
/// puts it below and to the left, so its *top-right* corner lands on the anchor.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Gravity {
    Center,
    Top,
...
/// What the positioner may do when the surface does not fit. A bitmask rather
/// than an enum because the axes are independent.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ConstraintAdjustment(u32);

impl ConstraintAdjustment {
    /// Let it hang off the work area.
    ...
    /// Slide along both axes, and give up height before position. Deliberately
    /// no flipping: a panel that jumped above the bar it hangs from would read
    /// as a different widget.
...
impl Positioner {
    /// A panel hanging below a bar item, right-aligned to it so the two stay
    /// visually attached as other items come and go to the item's left.
...
/// order that preserves intent longest — and each step is skipped unless
/// [`Positioner::constraint_adjustment`] permits it.
///
/// The result is clamped into `work_area` regardless of policy, so a caller
/// never receives a rectangle it would have to re-check. A work area smaller
/// than the requested size yields a smaller, possibly empty, rect.
pub fn position(p: &Positioner, work_area: Rect) -> Rect {
    ...
    // Sliding an axis the surface cannot fit on pins it to an edge and destroys
    // the anchor for no gain; this guard leaves that case to resize.
...
mod tests {
    ...
    /// A screen with the bar's strip excluded. Inset on the left, right and
    /// bottom but not the top: the gap below the bar is the positioner's
    /// `offset`, and insetting here too would count it twice.
    ...
    fn contains_agrees_at_every_pixel() {
        ...
        // The bar strip is never inside, so a click on the item that opened the
        // panel reaches the bar's own hit test.
