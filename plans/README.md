...
| [FRAMEKERNEL_PLAN.md](./FRAMEKERNEL_PLAN.md) | 4-phase rip-and-replace plan to redesign SlopOS as an async-first framekernel with a Verus-verified OSTD critical path. Inspired by Asterinas (USENIX ATC '25). **Phase 2 closed** (status `phase-3-ready`): all 17 non-OSTD kernel crates carry `#![forbid(unsafe_code)]`; TCB ratio 0.722 % (≤ 1 % target); `just check-framekernel` six sub-gates green (KernMiri 407 pass / 29 ignored); 2458 kernel + userland tests pass; CVSS findings SLOPOS-2026-0001..0005 fixed. § 2J (performance verification) was dropped — no pre-Phase-1 baseline existed to gate against. |
