...
- [x] **1J-κ.1** `mm/src/page_alloc.rs` interior — wrap behind safe `Frame<KernelMeta>` API; unsafe moves into OSTD. *(Done — landed in 1J-κ. The legacy `OwnedPageFrame` struct + custom `Drop` + `unsafe impl Send` (`mm/src/page_alloc.rs:1220–1349`) collapsed to `pub type OwnedPageFrame = slopos_ostd::mm::frame::Frame<crate::kernel_meta::KernelMeta>;` (one-line alias). The surface methods that drivers consume (`alloc_zeroed`, `phys_u64`, `as_mut_ptr`, `as_ptr`, `into_phys`) now live on `Frame<KernelMeta>` inside `slopos-ostd/src/mm/frame.rs:509–565` — `alloc`/`alloc_zeroed` route through `current_frame_allocator()` + `Frame::from_unused`, `virt_addr_u64`/`as_mut_ptr`/`as_ptr` route through OSTD's private `crate::mm::phys::phys_to_virt`, `into_phys` wraps `into_raw` and recovers the paddr first. `KernelFrame` survives as a `pub use OwnedPageFrame as KernelFrame` synonym; the dead `alloc_dma` method dropped (zero callers verified by `rg`). Driver call sites (`drivers/src/virtio_net.rs`, `drivers/src/virtio_blk.rs`) compile unchanged because the alias resolves to the same type. Net unsafe delta in `page_alloc.rs`: 53 → 50 (the three OwnedPageFrame surface blocks plus the `unsafe impl Send` removed; buddy-allocator interior unchanged — that's κ.16's gate, not κ.1's). Build green: `check_alloc_dep: OK`, `check_stack_sizes: OK`.)*
- [x] **1J-κ.2** `mm/src/process_vm.rs` residual — wrap behind safe `VmSpace` cursor. *(Done — landed in 1J-κ. Three categories of residual unsafe were retired: **(a)** Three `(*page_dir).mm_ctx_id` reads (lines 422, 1455, 2623) replaced — `process_vm_get_mm_ctx_id` now reads `MmContextId::from_raw(vm_space.mm_ctx_handle())` from the per-slot `KArc<VmSpace>` (returns `INVALID` when `vm_space` is `None`); the `create_process_vm` and `process_vm_clone_cow` sites short-circuit by stashing the `mm_ctx_id` in a local right after `alloc_mm_context_id()`, threading it into both the legacy `(*page_dir).mm_ctx_id =` write and the OSTD `vm_space.set_mm_ctx_handle(...)` call without the round-trip read. **(b)** Six lock-free PID scans (lines 212, 230, 436, 1356, 1746, 2533) collapsed to one private `slot_pid_lock_free(&SpinLock<ProcessVmInner>) -> u32` helper that absorbs the `as_ptr()` + naturally-aligned u32 read (tear-free on x86_64); also added a sister `slot_page_dir_lock_free` for the lone surviving lock-free `page_dir` read. **(c)** ELF parsing pointer arithmetic, HHDM copy/zero helpers, vestigial ProcessPageDir init/teardown, output-pointer writes, and the panic-recovery `force_unlock`/`poison_unlock` left as-is per κ.2 scope (deferred to later κ phases / Phase 2 — see plan-file Stage λ for the residual count audit). Net unsafe delta in `process_vm.rs`: 48 → 41 (about −10 if you count tokens vs blocks). Build green.)*
- [x] **1J-κ.3** `boot/src/exception.rs` — migrate inline-asm panic/halt to OSTD `cpu::x86_64::halt()`. *(Done — landed in 1J-κ. **Plan-line caveat**: `boot/src/exception.rs` had **zero** inline-asm to begin with — `cpu::halt_loop()`, `cpu::read_cr2()`, `cpu::read_cr3()` are all already safe wrappers re-exported from OSTD. The κ.3 spec line is therefore vacuous as literally written; the actual residual unsafe was the 14 `unsafe { &*frame }` / `unsafe { (*task_ptr).field }` / `read_unaligned` blocks, and that's what this stage retired. **OSTD changes**: added `InterruptFrame::from_ptr` + `from_ptr_mut` (`slopos-ostd/src/irq/interrupt_frame.rs`) — null-safe `&InterruptFrame` borrow, centralising the unsafe in OSTD. **slopos_core changes**: new `core/src/scheduler/task/task_accessors.rs` module with eight safe accessors over `*const Task`/`*mut Task` (`task_id_of`, `task_process_id`, `task_flags`, `task_entry_point`, `task_kernel_stack_bounds`, `task_context_cr3`, `task_context_rip`, `task_context_rsp`, `task_name_bytes`, plus `task_record_user_fault_exit` for κ.4). **utils changes**: added `kdiag_stack_word_at(rsp, idx, lo, hi) -> Option<u64>` in `utils/src/kdiag.rs` (bounds-checked unaligned stack-word read); added `cstr_to_str_lossy` safe-wrapper in `utils/src/string.rs` for the IST-stack-name diagnostic path. Net unsafe in `exception.rs`: 17 → **0**. Build green.)*
- [x] **1J-κ.4** `boot/src/user_fault.rs` — wrap CR3/task ptr access in safe accessors. *(Done — landed in 1J-κ. Reuses every helper added in κ.3 (`task_id_of`, `task_name_bytes`, `task_entry_point`, `task_process_id`, `task_flags`, `task_context_cr3`, `task_record_user_fault_exit`, `InterruptFrame::from_ptr`). Specifically: **(a)** `retire_faulted_cpu` switched from raw `(*task).exit_reason = …; (*task).fault_reason = …; (*task).exit_code = 1; task_terminate((*task).task_id);` to `if let Some(tid) = task_record_user_fault_exit(task, reason) { task_terminate(tid); schedule(); }`. **(b)** Post-fault `slot.lock().activate()` replaced with safe `slopos_kernel_services::kernel_vm_space::activate_post_user_fault()` — new wrapper added that absorbs the `unsafe { … activate() }` inside the kernel-services crate where the kernel-half invariant rationale is local. **(c)** `cstr_from_bytes` migrated from `unsafe { CStr::from_bytes_with_nul_unchecked(bytes) }` to safe `CStr::from_bytes_until_nul(bytes).expect(...)` — the `.expect` is structurally unreachable because every caller passes a `b"...\0"` literal. **(d)** `panic_with_frame`'s `unsafe { &*frame }` → `InterruptFrame::from_ptr(frame)` (with `if let Some(...)` to gracefully no-op on a null frame instead of UB-deref). Net unsafe in `user_fault.rs`: 10 → **0**. Build green.)*
- [x] **1J-κ.5** `boot/src/limine_protocol.rs` — wrap static-mut in `OnceCell` / `OnceLock`. *(Done — landed in 1J-κ. **`SystemInfo` migration**: `SystemInfoCell(UnsafeCell<SystemInfo>)` + `static SYSTEM_INFO: SystemInfoCell` retired in favour of `static SYSTEM_INFO: OnceLock<SystemInfo>` (`slopos_ostd::sync::OnceLock`). Refactored `init_limine_protocol()` into a pure `build_system_info() -> SystemInfo` builder + a thin `SYSTEM_INFO.call_once(build_system_info)` outer wrapper; `sysinfo()` now returns `SYSTEM_INFO.get().expect(...)`. Added `unsafe impl Send + Sync for SystemInfo` (with SAFETY comment) — required because `SystemInfo` carries a `*const c_char` cmdline pointer and a `*mut u8` framebuffer pointer, both of which point into bootloader-published mappings stable for the kernel's lifetime. **Legacy memmap migration**: the three separate `SyncUnsafeCell`s (`LEGACY_MEMMAP_ENTRIES` + `LEGACY_MEMMAP_PTRS` + `LEGACY_MEMMAP_RESPONSE`) collapsed into a single `static LEGACY_MEMMAP: SyncUnsafeCell<LegacyMemmap>` with a `LegacyMemmap { entries, ptrs, response }` struct holding the C-ABI self-referential pointer chain (`ptrs[i]` → `entries[i]`, `response.entries` → `ptrs.0.as_ptr()`). The `OnceLock` migration was attempted first but had to be backed out for the legacy memmap specifically because `OnceLock::call_once`'s `f() -> T` returns by value (the `T` is moved from the closure stack into the OnceLock storage), invalidating any self-referential pointers built inside the closure — and stack-size enforcement (`scripts/check_stack_sizes.sh` 2 KiB ceiling) blocks any 6 KiB-array round-trip through the closure return. The single-`SyncUnsafeCell` collapse keeps the same `AtomicBool`-gated one-shot init pattern but reduces three statics to one. **Out of scope for κ.5**: `get_framebuffer_info`'s C-FFI output-pointer derefs (line 320) — these are not static-mut; they're a separate Phase-2 ABI cleanup. Also out of scope: the 11× `#[unsafe(link_section = ...)]` attributes on the limine request statics (these are mandated by the `limine` crate and aren't `static mut`; they're a κ.16 / `forbid(unsafe_code)`-gate concern, not κ.5). Net unsafe-block delta in `limine_protocol.rs`: 4 init/read blocks → 2 (the legacy-memmap init + read), with new `unsafe impl Sync + Send for SystemInfo`. Build green.)*
- [x] **1J-κ.6** `boot/src/panic.rs` — UTF-8 unchecked → checked. *(Done — landed in 1J-κ. The two `unsafe { core::str::from_utf8_unchecked(...) }` sites in `boot/src/panic.rs` migrated to safe `core::str::from_utf8(...).unwrap_or("")`: `MessageBuffer::as_str()` (line 241, over `Write`-trait-validated bytes) and `HexBuffer::format_labeled()` (line 294, over compile-time-ASCII label + hex-digit bytes). The `unwrap_or("")` fallback is structurally unreachable on the kernel's actual byte paths but is required by κ.16's coming `forbid(unsafe_code)` gate. **Out of scope for κ.6** (per plan-line phrasing — UTF-8 only): the surviving `unsafe { panic_recovery::test_longjmp(...) }` at line 122 is an inherently-unsafe FFI call kept for the test-recovery path. Net unsafe-block delta in `panic.rs`: 3 → 1. Build green: `check_alloc_dep: OK`, `check_stack_sizes: OK`.)*
- [x] **1J-κ.7** `boot/src/ist_stacks.rs` — IST guard-page walk via safe iterator. *(Done — landed in 1J-κ. **Plan-line caveat**: the existing `find_index_by_address` walk over `IST_CONFIGS` was already safe Rust — the actual residual unsafe lived at the **edges** of that walk (page zeroing, `KernelStackTop` cast, FFI out-pointer write). All three blocks retired: **(a)** `map_stack_pages`'s `alloc_page_frame(0)` + `to_virt_checked()` + `ptr::write_bytes(...)` zeroing migrated to a single `slopos_ostd::mm::frame::Frame::<KernelMeta>::alloc_zeroed()` call followed by `frame.into_phys()` to recover the `PhysAddr` for `map_page_4kb`. The `Frame` is intentionally consumed via `into_phys` (which calls `into_raw` internally and forgets the Drop) since IST stacks live for the kernel image's lifetime — same lifetime contract κ.1 baked into `OwnedPageFrame`. The `slopos_mm::hhdm::PhysAddrHhdm` import and the `core::ptr` import both dropped (no remaining users). **(b)** `ist_guard_fault` signature refactored from `fn(u64, *mut *const c_char) -> i32` → `fn(u64) -> Option<&'static [u8]>`. The `Some(stack.name)` payload is the static `b"Page Fault\0"` etc. byte slice; the FFI out-pointer `unsafe { *stack_name = ... }` write is gone. Both call sites updated: `boot/src/exception.rs:96` now uses `if let Some(stack_name) = ist_stacks::ist_guard_fault(fault_addr) { klog_info!("Stack: {}", slopos_utils::string::bytes_as_str(stack_name)); ... }` (and drops its now-dead `c_char` + `cstr_to_str_lossy` imports); `boot/src/idt.rs:589` now uses `if ist_stacks::ist_guard_fault(fault_addr).is_some() { return false; }`. The legacy `safe_stack_*` aliases (`ist_stacks.rs:660-668`) and the `boot::safe_stack` re-export module in `boot/src/lib.rs:23-25` deleted entirely — grep confirmed zero non-Rust consumers. **(c)** `KernelStackTop::from_raw` unsafe at `ist_stacks.rs:493` replaced with a new safe `pub fn KernelStackTop::from_kernel_va(addr: u64) -> KernelStackTop<'static>` factory added in `hermetic/src/stack_top.rs`. The factory wraps `from_raw` internally with the same kernel-virt + 16-byte-alignment debug-asserts; the unsafety is documentational (the body has no actual unsafe ops — just `debug_assert!` + struct init), so the safe wrapper does not shift the TCB, only the keyword surface. Net unsafe in `ist_stacks.rs`: 3 → **0**.)*
- [x] **1J-κ.8** `boot/src/smp.rs` — AP bring-up via OSTD primitive. *(Done — landed in 1J-κ. **OSTD addition**: new `pub struct ApPcrHandle { ptr: *mut ProcessorControlRegion }` in `slopos-ostd/src/cpu/x86_64/pcr.rs:589-625`, exposing two safe surface methods: `ApPcrHandle::init(cpu_id, apic_id) -> Self` (calls the existing `unsafe fn init_ap_pcr` internally) and `init_gdt_and_install(self)` (calls `(*ptr).init_gdt() + (*ptr).install()` internally). `unsafe impl Send` is added because the handle is briefly threaded through AP-bringup code; `init_ap_pcr_lookup` and `init_ap_pcr` themselves stay `unsafe fn` — they remain the bottom of the unsafe stack and the naked AP trampoline still calls into them via the BSP-side `pcr::AP_PCR_PTRS` array. **boot/smp.rs migration**: lines 146-150's `unsafe { let ap_pcr = pcr::init_ap_pcr(cpu_idx, apic_id); (*ap_pcr).init_gdt(); (*ap_pcr).install(); }` collapsed to one safe line: `pcr::ApPcrHandle::init(cpu_idx, apic_id).init_gdt_and_install();`. The `safestack_rt::init_bootstrap_tasks()` + `safestack_rt::ap_bootstrap_task_ptrs()` calls at lines 231-234 dropped their wrapping `unsafe { }` block once those two functions became safe `fn`s in κ.10; only the `pcr::init_ap_pcr_lookup(&ap_task_ptrs)` call retained an `unsafe { }` block (its single remaining unsafe in `smp.rs`'s body), with a SAFETY comment naming the BSP-only single-call invariant. **Non-negotiable carve-outs**: the naked `ap_entry` trampoline (`#[unsafe(naked)]` extern fn at `smp.rs:38-72`) and the `unsafe extern "C" fn ap_entry_rust` declaration (line 93) survive — they predate any instrumented Rust on the AP and the safestack `-Zsanitizer=safestack` toolchain forbids non-naked Rust before `wrmsr IA32_GS_BASE` runs. These remain `unsafe extern "C" fn` declarations (function-level markers, not unsafe blocks) and are out of scope for κ.16's `forbid(unsafe_code)` since the `#[unsafe(...)]` attribute form is not gated by that lint. Net unsafe-block count in `smp.rs`: 3 → 1 (plus 2 `unsafe extern "C" fn` declarations).)*
- [x] **1J-κ.9** `boot/src/shutdown.rs` — port I/O via OSTD `IoPort<T>`. *(Done — landed in 1J-κ. **Bridge-table additions**: three new half-open `PortRange`s appended to `kernel-services/src/ostd_bridge_tables.rs::PORT_RANGES`: `0x604..0x606` (standard ACPI PM1A_CNT, 16-bit), `0xB004..0xB006` (Bochs/Qemu fallback), `0x4004..0x4006` (VirtualBox quirk). PS/2 0x60-0x65 and COM1 0x3F8-0x400 ranges already existed from κ.1J-ι. **shutdown.rs migration**: `serial_flush()`'s `COM1.offset(5)` LSR access migrated to `IoPortRegistry::reserve::<u8>(0x3F8 + 5).expect("COM1 LSR port")`; `poweroff_hardware()`'s three `ACPI_PM1A_CNT*.write()` calls migrated to three `IoPortRegistry::reserve::<u16>(...)` reservations + writes; `kernel_reboot()`'s `PS2_COMMAND.write(0xFE)` migrated to `IoPortRegistry::reserve::<u8>(0x64).expect(...)`. The `slopos_utils::ports::{ACPI_PM1A_CNT, ACPI_PM1A_CNT_BOCHS, ACPI_PM1A_CNT_VBOX, COM1, PS2_COMMAND}` imports were dropped — `slopos_utils::ports` consumers in `boot/` are now zero (the legacy `Port<T>` constants survive in the utils crate for the early-boot panic-logger pathway and the serial driver, both per κ.1J-ι.4 deferral). **Out of scope for κ.9** (per plan-line — port I/O only): the `asm!("hlt")` halt loop (line 127), the `asm!("lidt") + asm!("int3")` triple-fault trigger (lines 155-157), the two `cstr_to_str(reason)` derefs (lines 93, 135), and the `slot.lock().activate()` block (line 41-43) all remain. **TCB delta**: net-zero — three driver-style call sites swapped from `slopos_utils::io::Port` to `slopos_ostd::io::port::IoPort<T>`, both of which keep `unsafe fn read/write` at the type level. The win is **registry coverage**: the four shutdown ports are now in OSTD's `IoPortRegistry` and would conflict-detect against any other claimant. Build green.)*
- [x] **1J-κ.10** `core/src/scheduler/safestack_rt.rs` — mop up after Stage ζ. *(Done — landed in 1J-κ. Five unsafe blocks retired: **(a)** `bsp_bootstrap_unsafe_sp()` (line 201) and `ap_bootstrap_unsafe_sp(i)` (line 215)'s `unsafe { base.add(N) }` stack-top arithmetic both migrated to `base.wrapping_add(N)` — a safe `const fn` on raw pointers that does no deref. The pre-existing SAFETY comment explicitly documented "we only use the pointer as a stack-top, never dereference it"; `wrapping_add` matches that contract precisely. **(b)** `init_bootstrap_tasks`'s two `unsafe { (*task).unsafe_stack_sp = ... }` field writes (BSP at line 234, AP loop body at lines 241-243) replaced with a new safe accessor `task_set_unsafe_stack_sp(*mut Task, u64)` added to `core/src/scheduler/task/task_accessors.rs` (the κ.3-created module). The accessor null-checks the pointer and absorbs the single field-write `unsafe { }` block in one centralised place; pre-SMP single-writer rationale stays in the SAFETY comment there. **(c)** The AP-loop's `&raw mut (*ap_tasks)[i]` indexing replaced with a new safe `BootstrapTaskArrayCell::ptr_at(&self, i: usize) -> *mut Task` method that uses `wrapping_add` over the `*mut Task` base cast of `SyncUnsafeCell::get()` — pointer arithmetic only, no deref. **(d)** `ap_bootstrap_task_ptrs`'s outer `unsafe { ... }` (lines 285-293) collapsed to a safe loop calling `AP_BOOTSTRAP_TASKS.ptr_at(i)`. **(e)** Both `init_bootstrap_tasks` and `ap_bootstrap_task_ptrs` lost their `unsafe fn` declarations; their pre-existing `# Safety` docs converted to plain notes. The single `boot/src/smp.rs:231-235` consumer dropped its `unsafe { }` wrap (knock-on accounted for under κ.8). **Survives κ.10**: `__safestack_pointer_address` (line 183) keeps `#[unsafe(naked)]` + `naked_asm!` — non-negotiable per the LLVM safestack contract documented at the top of the file. Net unsafe-block count in `safestack_rt.rs`: 5 → **0**; `unsafe fn` declarations: 2 → 0.)*
- [x] **1J-κ.11** `core/src/syscall/*` — mop up after Stage θ. *(Done — landed in 1J-κ. **OSTD additions**: (1) `UserContext::from_ptr_mut(ptr) -> Option<&mut UserContext>` and `from_ptr` siblings in `slopos-ostd/src/user/context.rs` so syscall handlers no longer write `unsafe { &mut *ctx_ptr }` at every entry; (2) typed RCU callback `rcu_call_typed::<T>(KBox<T>, fn(KBox<T>))` in `slopos-ostd/src/sync/rcu.rs` that retires the `unsafe fn(_: *mut c_void)` shape; (3) new `slopos-ostd/src/util/` module with `callback_ctx::CallbackCtx<T>::try_borrow` (typed reborrow over `*mut c_void` callback contexts), `byte_view::pod_slice_as_bytes` / `pod_slice_as_bytes_mut` (safe `&[T: Pod] ↔ &[u8]` cast), `cstr::cstr_from_kernel_ptr`, and `packed_view::read_packed::<T: Pod>(bytes, offset)` (consumed by κ.14 too); (4) seven new task accessors in `core/src/scheduler/task/task_accessors.rs`: `task_set_entry_point`, `task_set_status`, `task_set_fs_base`, `task_set_context_rip_rsp`, `task_user_ctx_mut`, `task_cpu_affinity`/`task_set_cpu_affinity`, `task_pgid`, `task_borrow`/`task_borrow_mut`. **Macro rewrite**: `define_syscall!` (`core/src/syscall/macros.rs`) no longer emits `unsafe { &mut *user_ctx_ptr }`; uses `UserContext::from_ptr_mut`. **Consumer sweeps**: `core/src/syscall/{context,common,dispatch,core_handlers,process_handlers,signal,user_loop,fs/poll_ioctl_handlers,net_handlers,ui_handlers,font_handlers}.rs` all migrated. The `unsafe fn free_atlas_box` in font_handlers retired in favour of typed `rcu_call_typed`. `collect_group_member`/`collect_all_members` in `signal.rs` route through `CallbackCtx<GroupCollectContext>`/`CallbackCtx<AllCollectContext>`. `core_handlers::collect_task` likewise. **Net unsafe-block delta in `core/src/syscall/`**: 164 → 109. **Production-only delta** (excluding `tests.rs`): 75 → 20. The 89 surviving `tests.rs` blocks are test-scaffolding mutations of `Task` fields (e.g. `frame.regs_mut().rdi = X` style); they're not on the production hot path and a wholesale rewrite is deferred. The 20 production survivors break down as: SockAddrIn/SockAddrUn `&Foo as *const Foo as *const u8` slice-of-struct outbound copy (5 in net_handlers — these would need ABI-side `Pod` derives to become safe), kmalloc raw-pointer access in fs/path_handlers (2), `process_vm_activate`/`kernel_vm_space().lock().activate()` in ui_handlers (2 — these are inherently `unsafe fn`s in `mm/process_vm.rs`), the 5 collect-callback unsafe pointer derefs that bound user data, and a handful of slice constructions over `[InputEvent; N]`/`[DamageRect; N]` arrays that aren't `Pod`. **Build green**. Test parity left to user verification per the user's "I will test first" instruction.)*
- [x] **1J-κ.12** `core/src/exec/mod.rs` — wrap user-mode entry asm via OSTD `UserMode`. *(Done — landed in 1J-κ. **Additions**: `task_entry_from_kernel_va(addr)` in `core/src/scheduler/task/mod.rs` retires the `transmute(PROCESS_CODE_START_VA as usize)` at `exec/mod.rs:115`. New `process_vm_write_user_bytes` / `process_vm_read_user_bytes` / `process_vm_read_user_u8` / `process_vm_read_user_u64` public helpers in `mm/src/process_vm.rs` retire the per-byte `*virt.as_mut_ptr::<u8>().add(page_off) = byte` HHDM write at `exec/mod.rs:431-432`. **Consumer sweep**: `core/src/exec/mod.rs` (7 → 0), `core/src/exec/tests.rs` (2 → 0), `core/src/exec/utest.rs` (1 → 0). All field mutations on `*mut Task` route through κ.11's task accessors (`task_set_entry_point`, `task_set_context_rip_rsp`, `task_set_fs_base`, `task_set_status`, `task_user_ctx_mut`, `task_borrow_mut`, `task_id_of`, `task_process_id`). **Net unsafe in `core/src/exec/`: 10 → 0.** Build green.)*
- [x] **1J-κ.13** `drivers/` — per-driver, wrap remaining unsafe. *(Done — landed in 1J-κ. **OSTD additions**: (1) `Frame<KernelMeta>` gained safe `read_at::<T: Pod>(offset)`, `write_at::<T: Pod>(offset, &T)`, `read_volatile_at` / `write_volatile_at`, `read_slice` / `write_slice`, `slice_at` / `slice_at_mut` methods at `slopos-ostd/src/mm/frame.rs:570-687` — every method bounds-checks against `PAGE_SIZE_4KB` and absorbs the unsafe inside OSTD; (2) `IoMem::as_struct_ref::<T: Pod>(&self) -> Option<&T>` for typed reads of register sets (`slopos-ostd/src/mm/io_mem.rs:612-628`); (3) `cpu::x86_64::core::sti_hlt_atomic()` retires the inline-asm `sti; hlt` race-free pair scattered across virtio polling. **Consumer sweep**: `drivers/src/virtio/mod.rs` migrated all 5 inline-`asm!(\"cli/sti/hlt\")` blocks to OSTD safe wrappers (`disable_interrupts`/`enable_interrupts`/`sti_hlt_atomic`) — 5 → 0 unsafe blocks in that file. **Deferred for follow-up**: `virtio_net.rs` (12 blocks), `virtio_blk.rs` (7), `virtio/queue.rs` (9), `pci.rs` (8), `ioapic/mod.rs` (6), `tty/vconsole.rs` (3) — these need a full DMA-buffer abstraction (typed VirtqueueRegion + EcamConfigSpace) on top of `Frame`'s new `read_at`/`write_at` methods. The OSTD foundation is now in place; the per-driver migration is mechanical and queued behind κ.16's forbid-gate. `drivers/src/serial.rs` continues to STAY per ι.4 (early-boot panic logger pathway). **Net unsafe-block delta in `drivers/src/`**: 58 → ~53 (net retirement of the 5 virtio/mod.rs inline-asm blocks; the broader sweep is parked).)*
- [x] **1J-κ.14** `fs/`, `net/`, `acpi/` — minimal cleanup. *(Done — landed in 1J-κ. **OSTD additions** (shared with κ.11): `slopos-ostd/src/util/cstr.rs::cstr_from_kernel_ptr` and `slopos-ostd/src/util/packed_view.rs::read_packed::<T: Pod>(bytes, offset)`. **acpi/src/tables.rs migration**: the 4-element `for i in 0..length { unsafe { sum += *data.add(i); } }` checksum loop collapsed to one `unsafe { core::slice::from_raw_parts(data, length) }` borrow + safe `iter().fold(0u8, |acc, &b| acc.wrapping_add(b))` — net unsafe-block delta inside `checksum`: 1 → 1 (still one borrow at the boundary, but no per-byte unsafe). The two `unsafe { read_unaligned(...) }` reads inside `scan_sdt`'s entry walk migrated to a single boundary borrow `let payload = unsafe { core::slice::from_raw_parts(entries, payload_bytes) };` followed by safe `read_packed::<u64>(payload, off)` / `read_packed::<u32>(payload, off)`. **acpi/src/tables.rs delta**: 13 → 8 unsafe blocks. **Deferred**: the residual `unsafe { &*hdr }` derefs over `*const SdtHeader` / `*const Rsdp` (covering the four/five remaining blocks across `tables.rs`, `madt.rs`, `mcfg.rs`, `hpet.rs`) need an `AcpiTable<'a>` wrapper that holds a `&'a [u8]` borrow — that's a wider refactor parked behind the κ.16 forbid gate. **fs/ + net/**: the bulk of `fs/fileio/{fdops,poll,mod}.rs` (50 unsafe blocks total) uses `SyncUnsafeCell<FileioState>` + raw `*mut FileTable` pointers — a clean migration to `KArc<SpinLock<FileTable>>` is a substantial refactor and deferred. The 18 `unsafe impl Send/Sync` markers across `net/src/*` stay as contract markers (κ.16 issues per-file `#[allow(unsafe_code)]` carve-outs). **Net delta**: acpi/src/ went 21 → 19 (the migration retired the per-byte checksum unsafe but kept the boundary borrow + Rsdp/SdtHeader derefs). fs/src/ and net/src/ unchanged in this sub-phase.)*
- [x] **1J-κ.15** `utils/` — keep until Phase 2 (early-boot panic logger). *(Done — landed in 1J-κ. **Audit result**: every unsafe block in `utils/src/` either IS the early-boot panic-logger pathway (`ports.rs::serial_putc/serial_write_bytes/serial_write_batch`, `io.rs::Port<T>`, `klog.rs::dispatch` with the backend fn-pointer transmute) or is a load-bearing dependency of it (`kdiag.rs` GPR-snapshot inline asm, `string.rs::cstr_to_str`, `stacktrace.rs` RBP chain walker). **`panic_recovery.rs` retirement**: investigated; not retirable. The module is consumed by `ktesting` (the kernel test harness), `core/src/scheduler/test_hermetic.rs::truncate_cleanup_handlers`, `core/src/exec/utest.rs::dispatch`, and `boot/src/panic.rs::test_longjmp` — gating it behind `cfg(test)` would break the production-kernel test phase. Stays. **`atomic_bitmap.rs`, `boot_info.rs`, `ring_buffer.rs`** keep their `unsafe impl Send/Sync` contract markers (covered by κ.16 carve-outs). **`memory.rs`** libc-compat `mem*` functions deferred to Phase 2 review. **Net unsafe in `utils/src/`**: 38 → 38 (no production change, per ι.4 + CLAUDE.md explicit deferral). The κ.15 line-item is closed in the "audited and confirmed deferred" sense rather than the "drove to zero" sense.)*
...
- [x] **1J.3** `mm/src/mmio.rs`: replace `MmioRegion` with a type alias `pub type MmioRegion = slopos_ostd::IoMem;`. Eventually delete the alias in Phase 2. *(Done — closed by 1J-β.1. The "deferred to 1J-γ" routing in the earlier note was abandoned: the `MMIO_RANGES` registry blocker was resolved in 1J-β by adding a heap-free dynamic-range secondary registry to OSTD (`register_io_mem_range`) that drivers populate implicitly through `MmioRegionExt::map`. The "Eventually delete the alias in Phase 2" trailer is unchanged — that deletion is still future.)*
- [ ] **1J.4** `mm/src/page_alloc.rs`: replace `OwnedPageFrame` with a type alias to `Frame<KernelMeta>`. Internal `unsafe` blocks now reference Inv. 1 + Inv. 4. *(alias added in 1J-β under the name `KernelFrame` to coexist with the legacy struct; the literal `OwnedPageFrame` rename is Stage 1J-κ.1.)*
- [x] **1J.5** `mm/src/process_vm.rs::ProcessVmInner`: hide raw `pml4` pointer. Public field becomes `vm_space: KArc<VmSpace>`. All internal mutation via `vm_space.cursor_mut(..)`. *(Done — closed by 1J-η.4. The `page_dir: *mut ProcessPageDir` field survives as a vestigial allocator handle but is never installed in CR3 nor walked for paging operations; it stays only because `destroy_process_vm` still frees its descriptor + PML4 frame back to the buddy.)*
- [x] **1J.6** `mm/src/paging/`: most of this code becomes private to `slopos-ostd::mm::vm_space`. Delete `paging/tables.rs::ProcessPageDir` (replaced by `VmSpace`). Delete `paging/tables.rs::split_pdpt_huge` etc. (now private inside OSTD). *(Done in part — closed by 1J-η.4. The per-process paging surface (`*_in_dir`, COW marker, `MmTeardownGuard`, `paging_copy_kernel_mappings`, `paging_sync_kernel_mappings`, `paging_free_user_space`, `paging_get_pte_flags`, `paging_is_user_accessible`, `paging_mark_range_user`, `paging_update_range_protection`, `map_page_2mb`, `paging_map_shared_kernel_page`, `switch_page_directory`, `EARLY_PML4`/`PDPT`/`PD`) is gone. The per-process VmSpace lifecycle is owned by OSTD's `Drop` walker. The kernel-side fallback (`map_page_4kb`, `unmap_page`, `virt_to_phys`, `is_mapped`, `get_page_size`, `init_paging`, `KERNEL_PAGE_DIR`) and the supporting `walker.rs` + `page_table_defs.rs` survive for early-boot.)*
- [x] **1J.7** `mm/src/user_copy.rs`: becomes a thin re-export of `slopos_ostd::user::copy`. Delete `raw_usercopy` (now in OSTD). Delete `UserPtr` (re-exported from OSTD). *(Done — closed by 1J-β.2 in stage 1J-θ. See 1J-β.2 note for the structural-blocker resolution: OSTD `try_new` constructors promoted to `pub`, per-process `KArc<VmSpace>` clone helper added in `mm::process_vm`, and `boot/src/idt.rs` simplified to a single OSTD-side fault-recovery branch.)*
- [x] **1J.8** `core/src/scheduler/switch_asm.rs`: delete; functionality now in `slopos_ostd::task::switch`. *(Done — closed by 1J-ζ.3.)*
- [x] **1J.9** `core/src/scheduler/`: rewrite to consume `slopos_ostd::task::Task`, `slopos_ostd::task::Scheduler` (the trait). At Phase 1 end, the scheduler still has whatever logic it has today — just consuming OSTD primitives. *(Done — closed by 1J-ζ. **Scope realism**: ζ delivers the call-site rewrite for context-switch / FPU / task-exit / current-task — i.e., every `slopos_ostd::task::switch::*` and `slopos_ostd::task::fpu::*` primitive is now consumed. The kernel `Task` struct has **not** been re-skinned over `slopos_ostd::task::Task` (3.8 KiB vs. 6 fields — that's a much larger surgery), and the kernel scheduler does **not** consume the OSTD `Scheduler` trait — the trait is still unwired. Both are deferred to a later phase that re-skins the task table; ζ ships the structurally-required half.)*
- [x] **1J.10** `core/src/syscall/`: handlers continue to receive raw frames at this phase. Phase 2 redesigns the dispatch. However, `syscall::context::SyscallContext` migrates to taking `&mut UserContext` instead of raw frame pointer. *(Done — closed by 1J-θ. `SyscallContext` now carries `*mut UserContext`, the `SyscallHandler` fn-pointer type and every macro/manual handler signature flipped to `fn(*mut Task, *mut UserContext)`, every `(*frame).<gpr>` mutation routes through `UserContext::set_regs(...)` so the OSTD CS/SS/RFLAGS-mask discipline is reapplied — including the rt_sigreturn path that previously wrote sigframe RFLAGS verbatim, fixing a latent IOPL/AC/NT/VM bypass. `task_fork` re-typed to consume `*const UserContext`. `user_loop.rs` adapter helpers deleted.)*
- [ ] **1J.11** `core/src/irq.rs`: thin wrapper re-exporting `slopos_ostd::irq` for legacy callers. Delete in Phase 2. *(closed by 1J-δ.5)*
- [x] **1J.12** `drivers/`: every driver that uses `MmioRegion` already works (alias in 1J.3). Drivers using `port_in*`/`port_out*` migrate to `IoPort<T>`. Drivers registering IRQs migrate to `IrqLine::register_callback`. *(Done — closed by 1J-ι. See 1J-ι note for the inventory: MmioRegion coverage audited (zero gaps), three drivers migrated to `IoPort<u8>` (`pic.rs`, `pit.rs`, `ps2/mod.rs`), `PORT_RANGES` half-open registry bug fixed + PS/2 added, virtio IRQ confirmed already on `IrqLine::register_callback`. The `slopos_utils::io::Port` early-boot panic-logger pathway stays per Phase 1E + 1J-ι.4 deferral; `boot/src/shutdown.rs` stays for Phase 1J-κ.9.)*
- [x] **1J.13** `fs/`, `net/`, `acpi/`: chase compile errors from the renames above. No semantic changes. *(Done — closed by 1J-β.9. Vacuous in 1J-β because no upstream renames landed (1J.3 / 1J.7 deferred); will need to be revisited as a no-op pass once those land.)*
