...
> **Status**: Phase 1 in progress — 1A (crate skeleton), 1B (`Frame<M>`), 1C (`UFrame` / `USegment`), 1D (`VmSpace` + cursor), 1E (`IoMem` / `IoPort` / `Dma*`), 1F (`IrqLine` / `IdtBuilder` / `DisabledPreemptGuard`), 1G (`UserContext` / `UserMode` / typed user copy), 1H (`KernelHeap` folded into ostd), 1I (sync primitives + `Task` primitive), 1J-α (wiring foundation: `register_*` hooks), **1J-β (safe aliases — partial: 1J-β.3..β.9; β.1 and β.2 deferred on prerequisite blockers)**, 1J-γ (port karch into OSTD + dep inversion), 1J-δ (IDT/GDT migration), 1J-ε (UserModeBackend + LSTAR), and 1J-ζ (scheduler/task migration onto OSTD) complete; **1J-η (VmSpace/paging migration — in progress: η.5 / η.6 done, η.1 dual-allocation half done, η.1 reader-flip + η.2 + η.3 + η.4 pending — see §5.1J-η for the 7-step migration plan and OSTD foundation extensions that landed)**; **1J split into 11 sub-phases (1J-α..1J-λ) — see §5.1J for the breakdown**; the remaining 1J-β subtasks roll into later sub-phases as their blockers clear.
...
- [ ] **1J-β.1** **(closes 1J.3)** `mm/src/mmio.rs`: replace body with `pub type MmioRegion = slopos_ostd::IoMem;`. Add extension trait if any method is OSTD-missing. *(**Deferred** — blocked on `MMIO_RANGES` registry expansion. 1J-α.1 populated `MMIO_RANGES` with the LAPIC range only; HPET/IOAPIC/PCI ECAM/framebuffer/virtio MMIO are deferred per spec. `IoMemRegistry::reserve` rejects un-registered ranges with `NotReserved`, so a literal type-alias replacement breaks every driver call site that constructs an `MmioRegion` for a non-LAPIC region. Roll into 1J-γ once the dynamic registry hook lands.)*
- [ ] **1J-β.2** **(closes 1J.7)** `mm/src/user_copy.rs`: replace body with `pub use slopos_ostd::user::copy::*;` and `pub use slopos_ostd::user::ptr::{UserPtr, UserSlice, UserBytes, UserPtrError, UserVirtAddr};`. Delete legacy `raw_usercopy`. *(**Deferred** — blocked on (a) OSTD's `UserPtr::try_new` / `UserSlice::try_new` / `UserVirtAddr::try_new` being `pub(crate)` (Inv. 5: only path is `UserContext::user_ptr_arg`), and (b) OSTD's `copy_from_user` etc. requiring `&VmSpace`. ~30 caller sites in `core/src/syscall/**` use `UserPtr::<T>::try_new(addr)` directly, and there is no per-process `&VmSpace` adapter yet (Stage 1J-η wires that). Roll into Stage 1J-θ once `UserContext`-based dispatch is the carrier.)*
...
**Status: in progress.** Three landed sub-stages of seven; 2409 kernel + 215 OSTD tests passing throughout. Detailed status:
...
- [~] **1J-η.1** `mm/src/process_vm.rs:27`: add `vm_space: KArc<VmSpace>`. *(Half done — dual-allocation landed; reader-flip pending. `ProcessVmInner` gained `vm_space: Option<KArc<VmSpace>>`. `create_process_vm` and `process_vm_clone_cow` allocate a fresh `VmSpace` via `VmSpace::new()` alongside the legacy ProcessPageDir, plumb the slopos `mm_ctx_id` via `vm_space.set_mm_ctx_handle(...)` (so the registered LUF / on-activate hook surface routes per-process state), and wrap the result in `KArc::try_new`. Failure paths roll back symmetrically. `destroy_process_vm` drops the OSTD KArc before freeing the legacy frames. **Pending:** flip `process_vm_get_cr3_phys` (L239) and `process_vm_find_pid_by_cr3` (L278) to read `vm_space.pml4_paddr()`. The flip is gated on η.3 dual-writing every user mapping into the OSTD VmSpace — otherwise the OSTD PML4 holds only kernel-half mappings and CR3 install immediately page-faults the user.)*
- [ ] **1J-η.2** Migrate the 9 `pml4` sites: L239, L278, L1251, L1270, L1271, L1333, L1363, L1437, L2194-2220 → `vm_space.cursor_mut(...)`. *(Pending. The PML4 alloc / zero / kernel-half copy sites at L1251 / L1270 / L1271 / L2194-2220 are subsumed by `VmSpace::new()` and can collapse to nothing once η.3 + η.4 complete. The free / rollback sites at L1333 / L1363 / L1437 retire in η.4 with `paging_free_user_space`.)*
- [ ] **1J-η.3** `mm/src/{demand,cow}.rs`, `core/src/exec/mod.rs`, `core/src/scheduler/scheduler.rs:267,440,453`, and `mm/src/process_vm.rs` internal helpers (`map_user_range`, `teardown_inner_mappings`, etc.): rewrite to consume `vm_space.cursor_mut(...)` instead of `*mut ProcessPageDir`. **Plus** new `mm/src/mmu/luf_hook.rs` registers `LufHook: CursorUnmapHook` with OSTD via `kernel-services::ostd_bridge`. *(Pending — see "Remaining work" expansion below for the migration-step list and the file-by-file rewrite.)*
- [ ] **1J-η.4** **(closes 1J.6)** Delete `mm/src/paging/{tables, walker, page_table_defs, mod}.rs`, `mm/src/paging_defs.rs`, `mm/src/mmu/cr3.rs::Cr3Value`, `mm/src/mmu/asid.rs::install_mm_context_cr3`, `MmTeardownGuard`, `KERNEL_PAGE_DIR`, `EARLY_PML4` / `EARLY_PDPT` / `EARLY_PD`. Migrate kernel-side helpers (`map_page_4kb`, `unmap_page`, `virt_to_phys`, `is_mapped`, `get_page_size`, `paging_map_shared_kernel_page`) to `kernel_vm_space().lock().cursor_mut/cursor()` callers (kernel_heap, mmio, stack_va, memory_init, io_mem_mapper_shim, mmu/mapping). *(Pending.)*
...
- [ ] **1J.3** `mm/src/mmio.rs`: replace `MmioRegion` with a type alias `pub type MmioRegion = slopos_ostd::IoMem;`. Eventually delete the alias in Phase 2. *(originally scheduled for 1J-β; **deferred** to 1J-γ — see 1J-β.1 note above for the `MMIO_RANGES` registry blocker.)*
...
- [ ] **1J.7** `mm/src/user_copy.rs`: becomes a thin re-export of `slopos_ostd::user::copy`. Delete `raw_usercopy` (now in OSTD). Delete `UserPtr` (re-exported from OSTD). *(originally scheduled for 1J-β; **deferred** to 1J-θ — see 1J-β.2 note for `pub(crate)` ctor + missing per-process `&VmSpace` adapter blockers.)*
