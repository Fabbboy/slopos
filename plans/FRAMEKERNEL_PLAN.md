...
**Goal.** **(closes 1J.14, 1J.16)** Drive `unsafe` outside `slopos-ostd/` and `slopos-utils/` to literal zero, except for an enumerated set of irreducible carve-outs (CPU-contract inline asm, naked-fn ABI trampolines, `unsafe impl GlobalAlloc`, `unsafe impl Send/Sync` over device pointers, legacy C-ABI self-references, BSP-only init). All survivors are catalogued in `docs/UNSAFE_CARVE_OUTS.md` and gated by `scripts/check_unsafe_carve_outs.sh`. Multi-week, runs in parallel as a series of small PRs.

**Stage map.** κ.1..κ.15 retired the **structural** unsafe across mm/, boot/, drivers/, fs/, acpi/. κ.17 adds the missing OSTD primitives (`VirtqueueRegion`, `EcamConfigSpace`, `AcpiTable<'a>`, expanded `task_accessors`, `IntrusiveLinkedList`). κ.18..κ.22 migrate the surfaces κ.1..κ.15 explicitly deferred. κ.23 builds the carve-out catalog. κ.16 flips the `forbid(unsafe_code)` gate on every non-OSTD non-`slopos-utils` `lib.rs`. 1J-λ is the close gate.

`slopos-utils/` is **out of scope** for the κ.16 forbid per Phase 1E + CLAUDE.md (early-boot panic-logger pathway); it retires in Phase 2.
...
- [ ] **1J-κ.17.1** Add `slopos_ostd::dma::VirtqueueRegion<T: Pod>` over `Frame<KernelMeta>`. Methods: `desc(idx) -> &T`, `desc_mut(idx) -> &mut T`, `slice_payload(idx, len) -> &[u8]`, all bounds-checked. Retires the virtqueue ring pointer arithmetic in `drivers/src/virtio_{net,blk}.rs` and `drivers/src/virtio/queue.rs`.
- [ ] **1J-κ.17.2** Add `slopos_ostd::pci::EcamConfigSpace` over `IoMem`. Methods: `read::<T: Pod>(bdf, offset)`, `write::<T: Pod>(bdf, offset, value)`. Retires the raw pointer arithmetic at `drivers/src/pci.rs` ECAM access sites.
- [ ] **1J-κ.17.3** Add `slopos_ostd::acpi::AcpiTable<'a>` holding `&'a [u8]`. Methods: `header() -> &SdtHeader`, `payload() -> &[u8]`, checksum-validated `from_bytes(slice) -> Option<Self>`. Retires `*const SdtHeader` / `*const Rsdp` derefs in `acpi/src/{tables,madt,mcfg,hpet}.rs`.
- [ ] **1J-κ.17.4** Add `slopos_ostd::dev::DeviceHandle::from_ptr` (parallel to `InterruptFrame::from_ptr`) — null-safe `&DeviceHandle` borrow. Used by `drivers/src/virtio_net.rs:169` and similar.
- [ ] **1J-κ.17.5** Extend `core/src/scheduler/task/task_accessors.rs` with the missing scheduler-hot-path fields: `task_time_slice` / `task_set_time_slice`, `task_next_ready` / `task_set_next_ready`, `task_inc_ref` / `task_dec_ref`, `task_fpu_state_mut`. Each absorbs one `unsafe { (*task).<field> }` per-field access pattern.
- [ ] **1J-κ.17.6** Add `slopos_ostd::sync::IntrusiveLinkedList<T>` — safe wrapper over the `next_ready: *mut T` pattern in `core/src/scheduler/per_cpu.rs::ReadyQueue`. Methods: `push`, `pop`, `remove(handle)`, `iter()`. Implementation may keep one internal `unsafe`; consumers see a safe surface.
...
- [ ] **1J-κ.18.1** `core/src/scheduler/scheduler.rs` (~64 → 0 + ≤4 carve-outs). Migrate every `(*task).<field>` access to a `task_accessors::*` call. PCR pointer casts (lines 70–71, 158, 200, 238) already route through the safe `slopos_arch::pcr::*` API — drop the `unsafe { }` wrap.
- [ ] **1J-κ.18.2** `core/src/scheduler/per_cpu.rs` (~56 → 0 + ≤2 carve-outs). Replace `ReadyQueue` linked-list mutations (lines 76–79, 100, 112, 117, 122, 133–141, 155, 162, 165) with `IntrusiveLinkedList` (κ.17.6). Per-CPU `SyncUnsafeCell` access already guarded by `PreemptGuard` — wrap in a safe inherent method.
- [ ] **1J-κ.18.3** `core/src/scheduler/runtime.rs` (~8 blocks; ≤6 carve-outs). All inline asm (HLT/CLI/STI on lines 140, 237–248, 279, 289, 428, 448) is irreducible — wrap each in `#[allow(unsafe_code)]` + SAFETY comment citing the CPU contract. The `transmute(entry as *const ())` at lines 99, 188, 346 retires via a typed `TaskEntry` newtype.
- [ ] **1J-κ.18.4** `core/src/scheduler/task/task_lifecycle.rs` (~9 → 0 + ≤2 carve-outs). Task-creation field writes route through κ.17.5 accessors. The `unsafe extern "C"` declaration at line 401 (LLVM intrinsics ABI) is irreducible — carve-out.
- [ ] **1J-κ.18.5** `core/src/scheduler/task/task_table.rs` (~12 → 0). Replace `ZombieList` raw-pointer intrusive list with `IntrusiveLinkedList<Task>` (κ.17.6). Task-pool slot indexing already bounds-checked — drop the `unsafe { }`.
- [ ] **1J-κ.18.6** `core/src/scheduler/task_struct.rs` — already clean; offset-of asserts at lines 119–120 are compile-time, no runtime unsafe.
...
- [ ] **1J-κ.19.1** `mm/src/kernel_heap.rs` (~16 → 0). Wrap slab-header / large-alloc-header pointer arithmetic in safe inherent methods on `SlabHeader` / `LargeAllocHeader`; the unsafe collapses to one `Frame::slice_at_mut` call per allocation path.
- [ ] **1J-κ.19.2** `mm/src/memory_reservations.rs` (~10 → 0). Replace `ptr::copy()` array-store manipulation (lines 80, 98, 114, 135–143) with safe slice operations on the backing buffer; lazy-init via `OnceLock<SpinLock<RegionStore>>`.
- [ ] **1J-κ.19.3** `mm/src/page_alloc.rs` interior (~50 → 0 + ≤3 carve-outs). Add `Frame<KernelMeta>::at_index(frame_num)` safe wrapper around `FRAMES_PTR + bounds-check + dereference`. Per-CPU cache `pcp_cache` already guarded by `PreemptGuard` — drop the wrap. Buddy free-list pointer chasing collapses to one `unsafe` block per allocator method, gated by SAFETY note citing Inv. 1 + Inv. 4 (or moves into OSTD via a new `slopos_ostd::mm::buddy::BuddyAllocator` if needed — pick during execution).
- [ ] **1J-κ.19.4** `mm/src/process_vm.rs` residual (~41 → 0 + ≤2 carve-outs). HHDM phys-to-virt walks at lines 1227, 1244, 1469, 2646 route through `Frame::read_at` / `write_at`. The vestigial `*mut ProcessPageDir` field writes (lines 37–73) are deletable now that `vm_space` is the sole truth.
- [ ] **1J-κ.19.5** `mm/src/memory_init.rs` (~14 → 0 + ≤2 carve-outs). `SyncUnsafeCell` boot-time statistics access wraps in a safe `OnceLock<MemoryInitState>`. Limine response pointer dereference is BSP-only init — carve-out.
- [ ] **1J-κ.19.6** `mm/src/stack_va.rs` (~2 → 0). Per-CPU cache pattern already safe — drop the `unsafe { }` at lines 377–383, 406–409, 432, 478, 601.
- [ ] **1J-κ.19.7** `mm/src/lib.rs` GlobalAlloc impl (1 carve-out). `unsafe impl GlobalAlloc for BumpAllocator` is mandated by Rust trait — carve-out at lines 73–80.
- [ ] **1J-κ.19.8** Drop `#![allow(unsafe_op_in_unsafe_fn)]` from `mm/src/lib.rs` once κ.19.1..κ.19.7 close.
...
- [ ] **1J-κ.20.1** `boot/src/idt.rs` (~25 → 0 + ≤5 carve-outs). Frame-deref sites (lines 440, 572, 575, 587) → `InterruptFrame::from_ptr`. Task-pointer field reads → `task_accessors`. Handler-table `SyncUnsafeCell` reads (lines 93, 115, 133, 150, 193, 210, 240, 311, 430, 453) collapse to a single `OnceLock<[ExceptionHandler; 32]>` per table; 4–5 carve-outs survive at the BSP-only registration calls.
- [ ] **1J-κ.20.2** `boot/src/early_init.rs` (~17 → 0 + ≤3 carve-outs). Boot-state `SyncUnsafeCell` reads (lines 186, 190, 269) wrap in a safe `boot_state() -> &'static BootState` accessor. Page-table init writes (lines 524, 541, 553, 573) route through OSTD's safe `PhysAddr` / `VmSpace` constructors. BSP-pre-kernel-mode transitions are irreducible carve-outs.
- [ ] **1J-κ.20.3** `boot/src/limine_protocol.rs` — 3 sites are all carve-outs. The legacy memmap self-referential C-ABI cannot be retired without breaking the ABI; `unsafe impl Send + Sync for SystemInfo` is a contract marker over bootloader-published pointers.
- [ ] **1J-κ.20.4** `boot/src/shutdown.rs` ≤4 carve-outs. `asm!("hlt")` halt loop and `asm!("lidt") + asm!("int3")` triple-fault are irreducible. `slot.lock().activate()` migrates to `slopos_kernel_services::kernel_vm_space::activate_post_user_fault()` (κ.4 wrapper). `cstr_to_str` migrates to `slopos_ostd::util::cstr::cstr_from_kernel_ptr` (κ.11).
...
- [ ] **1J-κ.21.1** `drivers/src/virtio_net.rs` (~12 → 0 + ≤1 carve-out). DeviceHandle ptr borrow → `DeviceHandle::from_ptr`. Virtqueue ring access → `VirtqueueRegion<T>`. Frame payload access → `Frame::slice_at_mut`. The `unsafe impl Send` device contract marker is irreducible.
- [ ] **1J-κ.21.2** `drivers/src/virtio_blk.rs` (~7 → 0). Same migration as κ.21.1, no carve-outs needed.
- [ ] **1J-κ.21.3** `drivers/src/virtio/queue.rs` (~9 → 0 + ≤1 carve-out). Virtqueue init → `VirtqueueRegion<T>` constructor. The `unsafe impl Send for Virtqueue` contract marker is irreducible.
- [ ] **1J-κ.21.4** `drivers/src/pci.rs` (~8 → 0 + ≤2 carve-outs). ECAM config-space access → `EcamConfigSpace` (κ.17.2). The `unsafe impl Sync for PciDriver` and `unsafe impl Send for PciDriverRegistry` markers are load-bearing.
- [ ] **1J-κ.21.5** `drivers/src/ioapic/mod.rs` (~6 → 0). MMIO setup migrates to safe `MmioRegion` / `IoMem` calls (already alias post-1J-β.1).
- [ ] **1J-κ.21.6** `drivers/src/tty/vconsole.rs` (~3 → 0). Framebuffer MMIO → `IoMem::as_struct_ref()`.
- [ ] **1J-κ.21.7** `drivers/src/serial.rs` — explicit deferral via a file-wide `#[allow(unsafe_code)]` with SAFETY note pointing at the Phase 1E + κ.15 + ι.4 deferral chain.
...
- [ ] **1J-κ.22.1** `fs/src/fileio/{fdops,mod,poll}.rs` (~50 → 0). Refactor the `SyncUnsafeCell<FileTable>` + raw-`*mut FileTableSlot` pattern to `KArc<SpinLock<FileTable>>`. Substantial structural change; one PR scoped to fs/. No carve-outs expected.
- [ ] **1J-κ.22.2** `acpi/src/tables.rs` (~7 → 0 + ≤1 carve-out). Replace `*const SdtHeader` / `*const Rsdp` derefs with `AcpiTable::from_bytes(slice)`. The single boundary borrow `slice::from_raw_parts(data, length)` over the bootloader-published ACPI region is irreducible — carve-out at the `parse_rsdp` entry point.
- [ ] **1J-κ.22.3** `acpi/src/{madt,mcfg,hpet}.rs` (~6 → 0). Migrate to `AcpiTable<'a>` typed accessors.

**Acceptance.** `rg 'unsafe' fs/src/ acpi/src/ --type rust | grep -v tests | wc -l` ≤ 1.

##### κ.23 — Carve-out catalog

- [ ] **1J-κ.23.1** Create `docs/UNSAFE_CARVE_OUTS.md`. For every surviving `#[allow(unsafe_code)]` site after κ.17..κ.22 close, list:
  - file:line range
  - one-line reason: `inline-asm-cpu-contract` / `unsafe-trait-rust-abi` / `extern-c-abi-marker` / `unsafe-impl-send-device-pointer` / `bsp-only-init` / `naked-fn-llvm-contract` / `legacy-c-abi-self-ref` / `boundary-slice-borrow`
  - SAFETY-comment summary cross-referencing the OSTD soundness invariant (Inv. 1..10) it preserves.
- [ ] **1J-κ.23.2** Add `scripts/check_unsafe_carve_outs.sh` — diff `rg '#\[allow\(unsafe_code\)\]' --type rust -g '!slopos-ostd/**' -g '!slopos-utils/**' -g '!**/tests*'` against the catalog. Build fails on any extra or missing site.
- [ ] **1J-κ.23.3** Test files — add `#![cfg_attr(test, allow(unsafe_code))]` to the top of every `**/tests.rs` and `**/tests/**.rs` that contains test-scaffolding `unsafe`. Production lib stays under forbid.

**Acceptance.** `docs/UNSAFE_CARVE_OUTS.md` enumerates ≤ 60 sites total. `scripts/check_unsafe_carve_outs.sh` exits 0.

##### κ.16 — Forbid gate (final flip)

- [ ] **1J-κ.16** Add `#![forbid(unsafe_code)]` to `boot/src/lib.rs`, `mm/src/lib.rs`, `core/src/lib.rs`, `drivers/src/lib.rs`, `fs/src/lib.rs`, `net/src/lib.rs`, `acpi/src/lib.rs`, `karch/src/lib.rs`, `kernel-services/src/lib.rs`, `video/src/lib.rs`, `abi/src/lib.rs`. Apply `#[allow(unsafe_code)]` ONLY at the sites enumerated in `docs/UNSAFE_CARVE_OUTS.md` (built in κ.23). `slopos-utils/` is out of scope (Phase 1E + CLAUDE.md).

**Acceptance.** `just build && just test` green; `rg '#\[allow\(unsafe_code\)\]' --type rust -g '!slopos-ostd/**' -g '!slopos-utils/**'` line count matches `docs/UNSAFE_CARVE_OUTS.md`.
...
κ.17 (primitives)
  ...
  ├─ κ.19 (mm tail)
  ├─ κ.20 (boot residual)
  ├─ κ.21 (drivers tail)        ─→ κ.23 (catalog) ─→ κ.16 (forbid) ─→ 1J-λ
  └─ κ.22 (fs/acpi tail)
```

κ.18..κ.22 may run in parallel once κ.17 lands. κ.16 is gated by κ.23.

**Verify (overall).** `rg '\bunsafe\b' --type rust -g '!slopos-ostd/**' -g '!slopos-utils/**' -g '!userland/**' -g '!slibc/**' -g '!slop-protocol/**' -g '!ktesting/**' -g '!*.s' -g '!**/tests*'` returns only lines that have a matching `#[allow(unsafe_code)]` attribute documented in `docs/UNSAFE_CARVE_OUTS.md`.
...
- [ ] **1J-λ.2** `just check-framekernel` clean (κ.16 forbid + carve-out diff + check_alloc_dep + check_stack_sizes).
...
- [ ] **1J-λ.6** TCB ratio: count `\bunsafe\b` tokens in `slopos-ostd/`, divide by total kernel LoC. Target ≤ 1.5%.
