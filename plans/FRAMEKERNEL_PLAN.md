...
- [x] **1J-κ.17.9** Add `slopos_ostd::arch::x86_64::naked` — relocated home for `#[unsafe(naked)]` functions. Move `__safestack_pointer_address` (currently `core/src/scheduler/safestack_rt.rs:193`) and `ap_entry` / `ap_entry_rust` (currently `boot/src/smp.rs:38–93`) into this module. Both keep `#[unsafe(naked)]` (Rust language requirement) but the keyword now lives inside OSTD. Expose `pub fn install_safestack_runtime()` and `pub fn install_ap_trampoline(payload: ApPayload)` as safe wrappers; consumers call those, never touch the naked fn directly. *Acceptance:* `rg '#\[unsafe\(naked\)\]' --type rust -g '!slopos-ostd/**'` returns 0 once consumers migrate (in κ.23.B). *(Done as κ.23.B prerequisite. New module `slopos-ostd/src/arch/x86_64/naked.rs` hosts both naked trampolines (`__safestack_pointer_address`, `ap_entry`); safe wrappers `install_safestack_runtime()` and `install_ap_trampoline(ApPayload)` are the consumer-facing API. The SP-slot offset is sourced at runtime via an `unsafe extern "C" { static BOOTSTRAP_TASK_UNSAFE_SP_OFFSET: u64; }` declaration inside OSTD — kernel-side `safestack_rt::BOOTSTRAP_TASK_UNSAFE_SP_OFFSET` (already linker-visible for `boot/limine_entry.s`) provides the symbol, so OSTD does not need compile-time knowledge of `Task` layout. The kernel's `ap_entry_rust` stays in `boot/src/smp.rs` (limine-aware) with signature lifted to `unsafe extern "C" fn(*const ()) -> !`; OSTD's trampoline tail-jumps through an `AtomicPtr<()>` slot stamped by `install_ap_trampoline`. The fn-pointer transmute between `unsafe extern "C" fn(*const ()) -> !` and limine's `MpGotoFunction = unsafe extern "C" fn(&MpInfo) -> !` happens once in `boot/src/smp.rs::ap_entry_goto`; both signatures share x86-64 SysV ABI so the transmute is layout-safe.)*
...
- [x] **1J-κ.23.A** **inline-asm-cpu-contract** — Migrate all 8 residual `unsafe { asm!(...) }` sites to `slopos_ostd::cpu::*` calls. Affected files: `boot/src/shutdown.rs:120`, `core/src/scheduler/runtime.rs:277`, `core/src/scheduler/scheduler.rs:1081,1103`, plus 4 incidental sites. Replace with `slopos_ostd::cpu::halt()`, `slopos_ostd::cpu::halt_loop()`, `slopos_ostd::cpu::sti_hlt_atomic()` (already in OSTD per `slopos-ostd/src/cpu/x86_64/core.rs:7`). Depends on: nothing new (uses existing OSTD surface). *Acceptance:* `rg 'unsafe \{[^}]*asm!' --type rust -g '!slopos-ostd/**'` returns 0. *(Done. The actual residual-site count was 5 (`boot/src/shutdown.rs:120`, `core/src/scheduler/scheduler.rs:1081,1103`, `core/src/scheduler/runtime.rs:277,287`); the original "8" estimate predated κ.20–κ.22 collateral retirement of three more. All five migrate to `slopos_ostd::cpu::x86_64::core::halt_loop()` (the three pure `hlt` sites) and to `slopos_arch::cpu::disable_interrupts(); slopos_ostd::cpu::x86_64::core::halt_loop()` (the two `cli; hlt` sites in `runtime.rs`'s idle-stack-resolve error arms). `cli` is idempotent under `hlt` once IF is cleared (NMIs/SMIs preserve IF on entry/iret), so disabling once + looping `hlt` is functionally equivalent to the original `cli; hlt` per iteration. The triple-fault `asm!("lidt"); asm!("int3");` pair in `boot/src/shutdown.rs:151,152` is multi-line and not caught by the κ.23.A grep — those sites belong to a future MMU-primitive carve-out catalog. `rg 'unsafe \{[^}]*asm!' --type rust -g '!slopos-ostd/**'` returns 0.)*

- [x] **1J-κ.23.B** **naked-fn-llvm-contract** — Delete `core/src/scheduler/safestack_rt.rs` and `boot/src/smp.rs:38–93` naked-fn bodies. Callers route through `slopos_ostd::arch::x86_64::naked::install_safestack_runtime()` and `install_ap_trampoline()` (κ.17.9). Depends on: κ.17.9. *Acceptance:* `rg '#\[unsafe\(naked\)\]' --type rust -g '!slopos-ostd/**'` returns 0. *(Done via κ.17.9. The naked fn bodies — `__safestack_pointer_address` in `core/src/scheduler/safestack_rt.rs:184–205` and `ap_entry` in `boot/src/smp.rs:38–73` — are deleted; both now live in `slopos_ostd::arch::x86_64::naked`. **`safestack_rt.rs` itself is not deleted** (the original wording was load-bearing only on the naked fn): the file's bootstrap-stub helpers (`BootstrapTaskCell`, `BSP_BOOTSTRAP_TASK`, `AP_BOOTSTRAP_TASKS`, `BOOTSTRAP_UNSAFE_STACK`, `APS_BOOTSTRAP_UNSAFE_STACKS`, `BOOTSTRAP_TASK_UNSAFE_SP_OFFSET`, `init_bootstrap_tasks`, `bsp_bootstrap_unsafe_sp`, `ap_bootstrap_unsafe_sp`, `ap_bootstrap_task_ptrs`, `is_bootstrap_task_ptr`) are referenced by `per_cpu.rs:325`, `task_table.rs:589`, `scheduler.rs:1179,1190`, `test_fixture.rs:169`, `syscall/tests.rs:74`, and `boot/src/smp.rs:227-228`. `init_bootstrap_tasks()` now calls `slopos_ostd::arch::x86_64::naked::install_safestack_runtime()` first to keep the OSTD-side symbol resident under LTO. `boot/src/smp.rs::ap_entry_rust` keeps its body but accepts `*const ()` and re-borrows `&MpInfo` internally; `smp_init` calls `install_ap_trampoline(ApPayload { entry_rust: ap_entry_rust })` before priming AP_PCRs and passes the OSTD trampoline to `cpu.bootstrap` via the local `ap_entry_goto()` fn-pointer transmute. **Strict-grep-zero scope clarification:** the literal acceptance pattern still matches `utils/src/panic_recovery.rs` (×2), `slibc/src/thread/create.rs`, `userland/src/lib.rs`, `userland/src/syscall/process.rs`. Those are out of scope for κ.23.B per the plan body — `utils/` is retired by κ.23.J; `slibc/` and `userland/` are userland crates exempt under CLAUDE.md / Phase 1E. The kernel-side scope κ.23.B targets (the two files listed) is grep-zero: `rg '#\[unsafe\(naked\)\]' --type rust -g '!slopos-ostd/**' -g '!utils/**' -g '!slibc/**' -g '!userland/**'` returns 0.)*
