...
- [x] **3G.6** Tests: 13 KTAP kernel tests in `ring/src/tests.rs` (region volatile round-trips, CQE post/overflow/reserve, in-flight table, `OP_CANCEL` pending/missing/cancel-all, `OP_NOP`, layout invariants) + 5 userland end-to-end subtests in `userland/src/bin/tests/ring_test.rs` (setup geometry, NOP, OP_WRITE/OP_READ pipe **parity**, **deferred read** via blocking harvest, future **cancellation**). All green (`just test`: 2485 pass).
