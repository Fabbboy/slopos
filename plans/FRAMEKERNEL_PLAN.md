...
> **Status**: Phase 1 in progress — 1A (crate skeleton), 1B (`Frame<M>`), 1C (`UFrame` / `USegment`), 1D (`VmSpace` + cursor), 1E (`IoMem` / `IoPort` / `Dma*`), 1F (`IrqLine` / `IdtBuilder` / `DisabledPreemptGuard`), 1G (`UserContext` / `UserMode` / typed user copy), 1H (`KernelHeap` folded into ostd), 1I (sync primitives + `Task` primitive), 1J-α (wiring foundation: `register_*` hooks), **1J-β (safe aliases — partial: 1J-β.3..β.9; β.1 and β.2 deferred on prerequisite blockers)**, and 1J-γ (port karch into OSTD + dep inversion) complete; **1J split into 11 sub-phases (1J-α..1J-λ) — see §5.1J for the breakdown**; the remaining 1J-β subtasks roll into later sub-phases as their blockers clear.
