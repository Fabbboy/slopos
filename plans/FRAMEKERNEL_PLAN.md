...
- [x] **3F.2** Each opcode justified against an existing sync syscall path in `SLOPRING.md` § 12 (opcode catalogue table): `OP_READ`→`file_read_fd`, `OP_WRITE`→`file_write_fd`, `OP_RECVMSG`→`unix_recvmsg`/`socket_recvfrom`, `OP_SEND`→`unix_send`/`socket_send`, `OP_ACCEPT`→`unix_accept`/`socket_accept`, `OP_POLL_ADD`→`file_poll_register_fd` (+ `file_poll_fused` to read readiness), `OP_TIMEOUT`→`wait_event_timeout` as the *harvest-wait deadline* (not an independently-firing op), `OP_CANCEL`→in-flight-table walk, `OP_NOP`→none. **No new blocking primitives** — each opcode runs the existing path's non-blocking *probe* (the path reads a *stored* nonblocking flag, not a per-call arg — § 12 cross-cutting reality 1) and, on EAGAIN, records an in-flight row that the harvest phase re-probes; R12 parity holds because it is the *same code path*. AF_INET (raw user ptrs) vs AF_UNIX (kernel slices) marshalling differs per family (§ 12 reality 2).
- [x] **3F.3** Threat model documented in `SLOPRING.md` § 13 (asset/boundary table + nine sub-sections): SQEs are hostile user-controlled bytes in concurrently-mutable shared memory; snapshot-then-validate closes TOCTOU; every user VA re-validated through `UserSlice::try_new`; ring memory is `UFrame<RingMeta>` so Inv. 4 + Inv. 5 hold by construction (no kernel `&T` over it); stale/forged ring fds resolve via the Phase-2H `HandleTable` to typed `-EBADF`, never UB; and the core claim — **no kernel reference into the ring outlives a single `ring_enter` invocation** (§ 13.7) — is argued from the volatile byte-copy-only access discipline. The review also surfaced (and § 13 now closes) the access-level data race under the old non-atomic byte interface, the CQ-overflow fd-leak, the `sq_entries`-clamp DoS, and the mmap-after-close UAF.

### 3G: io_uring-style ring surface — implementation

- [ ] **3G.0** *(prerequisite, in OSTD)* Add the volatile/atomic `UFrame` accessor (`load_u32_acquire` / `store_u32_release` / `copy_out_volatile` / `copy_in_volatile`) — the only new OSTD `unsafe` SlopRing needs (SLOPRING § 3, § 5.3). Reading concurrently-user-writable ring memory through the current non-atomic `read_pod`/`read_bytes` is a data race; this accessor is the fix. Each method carries a `// SAFETY:` note naming Inv. 4/5; added to the KernMiri scope. **TCB delta: bounded (a handful of lines), audited.**
- [ ] **3G.1** New crate `ring/` (kernel-side) carrying `#![forbid(unsafe_code)]`. Hosts the SQ/CQ snapshot (via the 3G.0 accessor), opcode dispatch, in-flight table, and the **per-ring `SpinLock`** (OSTD `sync::spin`; SLOPRING § 6.3 — serializes concurrent `ring_enter` on one ring fd / `dup`). Allocations via `slopos-ostd` discipline.
- [ ] **3G.2** Two new syscalls (slots `157`/`158`, `SYSCALL_TABLE_SIZE` → ≥159): `ring_setup(entries: u32, params: *mut RingParams) -> i32` (honest signature, no `flags`-slot pun — SLOPRING § 6.1) and `ring_enter(ring_fd, to_submit: u32, min_complete: u32, flags: u32) -> i32`. Implemented via `define_syscall!` — both **sync**. The submit loop clamps `n = min(to_submit, sq_entries, sq_tail - sq_head)` (§ 13.6); `ring_enter` preserves `n_submitted` on `EINTR` (§ 6.2).
- [ ] **3G.3** Per-ring kernel state stored in the `HandleTable` shape from Phase 2H (generation-counter handles, so stale ring FDs return a typed error, never UB). The ring VMA pins its `Frame<RingMeta>` so the region cannot be freed while user PTEs map it (no mmap-after-close UAF — § 14).
- [ ] **3G.4** Opcodes implemented. Each is one subtask (3G.4.{a..i}): `OP_READ`, `OP_WRITE`, `OP_RECVMSG`, `OP_SEND`, `OP_ACCEPT`, `OP_POLL_ADD`, `OP_TIMEOUT`, `OP_CANCEL`, `OP_NOP`. Each runs the sync path's **non-blocking probe** (handling the stored-flag and AF_INET/AF_UNIX-marshalling realities of § 12); ownership ops (`OP_ACCEPT`, consuming reads) **reserve a CQE slot before the side effect** (§ 11 — never leak an fd on CQ-full).
- [ ] **3G.5** `min_complete > 0` is the **caller-as-waiter multi-fd poll loop** (SLOPRING § 7.1/§ 8.3, the `core/src/syscall/fs/poll_ioctl_handlers.rs` shape): register the calling task on every *distinct* in-flight `KernelEvent` queue (`subscribe_current`, deduped, tracked via `file_poll_track_registrations` for kill-safety), re-probe; if still short, `block_current_task_with_timeout(deadline)` (the `sched`/`driver_runtime` primitive — **not** single-queue `Subscription::wait_event`, which binds one queue), re-probe on wake, post-block `has_pending_signal()` check; loop until `available_cqes() = cq_tail - cq_head >= min_complete`, a signal, or deadline; unregister (`remove_current` + `file_poll_clear_registrations`). The waiter is the user task; no kernel-side future, no `(ring, op)` callback.
- [ ] **3G.6** Test crate `ring/tests/`: KTAP-grammar tests for opcode parity (same observable result as the equivalent sync syscall), backpressure + CQ overflow (incl. the reserve-before-side-effect path: a full CQ must `-EAGAIN` an `OP_ACCEPT`, never orphan an fd), cancellation, ring-FD **closed-on-fork**, kill-during-`ring_enter` (→ `-EINTR` *and* submit-count preserved), concurrent `ring_enter` on one fd (per-ring lock), and the register-before-recheck lost-wakeup window.
