...
> **Status**: **COMPLETE — all three phases done; the framekernel plan is fully delivered (`status: complete`).** `slopos-ostd` owns every line of kernel `unsafe`; all 17 non-OSTD kernel crates are `#![forbid(unsafe_code)]`; TCB ratio 0.722 % (target ≤1 %). Three OSTD critical-path invariants (`Frame<M>` ref-count, slab/`HeapSlot` lifetimes, `VmSpace::cursor`) machine-checked under a pinned Verus toolchain (32 obligations); the io_uring-style SlopRing surface (`ring_setup` / `ring_enter`, nine opcodes — all real, incl. socket `OP_SEND`/`OP_RECVMSG` with SCM_RIGHTS) ships sync in the kernel with a userland runtime + `slopfut` executor, and `nc`'s TCP recv/send loop is ported onto it. No open SlopRing gaps or CVSS findings. `just test` 2485/2485; `just check-framekernel` green. Tagged `framekernel-phase-1` and `framekernel-phase-3` (phases 2–3 landed continuously on `develop`, so no separate `framekernel-phase-2` tag was cut — `framekernel-phase-3` marks the combined Phase-2+3 milestone). Per-subphase implementation notes live in `git log`.
> **Post-implementation bug-fix pass (2026-05).** A multi-angle adversarial review of the shipped 3H surface found and fixed four real defects the original landing missed: (1) the `RingParams` wire image serialized `region_addr` at byte 60 while the `#[repr(C)]` struct places it at 64 (u64-pad), so userland read `region_addr = region_bytes<<32` (`0x300000000000` for a 16-entry ring) and page-faulted `nc` on first ring access — fixed with `offset_of!`-driven `RingParams::to_bytes`/`from_bytes` + static offset asserts (`abi/src/ring.rs`); (2) `Errno::raw()` is already negative, so `opcode.rs`/`enter.rs` double-negated it — every `-EAGAIN` would-block was mis-read as a `+11` inline completion and every error CQE/return was positive, breaking *all* deferred completions and `nc`'s multiplexing; (3) the userland syscall asm wrappers clobbered only `rcx`/`r11`, so the compiler kept a zeroed `ymm3` across a kernel-SSE-clobbering syscall and reused it to zero-init an SQE → garbage opcode (a latent kernel-ABI bug affecting all userland; fixed by clobbering `xmm0-15` in `slibc/src/pal/raw.rs`); (4) `nc`'s `on_sock`/`on_stdin` re-armed forever on a persistent socket error (e.g. `ECONNRESET`) → 100 % CPU livelock — fixed to treat `res < 0` as terminal. Plus the executor `block_on` already-resolved guard, the `OP_ACCEPT` stored-nonblock-flag restore, and (5) a `ring_enter`↔`close` lock-order inversion that could deadlock the global ring registry under concurrent enter+close on a dup'd ring fd — fixed by decoupling the per-ring serialization lock from the global registry-table lock, which is now held only to clone a refcounted (`KArc`) ring handle, never across the fileio probe (`ring/src/registry.rs`). A follow-up pass then closed the last edge-case gaps: (6) `OP_WRITE`/`OP_SEND` deferred completions now get an exact wakeup — the socket `poll_fused`/`poll_unwait` enqueue/dequeue the TX-drained send-readiness event when `POLLOUT` is requested (`net/src/socket_file_ops.rs`, reusing the existing `sock_send_ev`); (7) `OP_SEND` and `OP_RECVMSG` are now real socket opcodes — `OP_SEND` routes to `socket_send`/`unix_send` (per-family, staged through a kernel buffer so no raw user VA reaches the send path), `OP_RECVMSG` parses the user `MsgHdr` and routes to `socket_recvfrom`/`unix_recvmsg` with full SCM_RIGHTS fd-passing (`ring/src/net_glue.rs`); and (8) CQ overflow is now visible — a sticky `SLOPRING_CQ_OVERFLOW` latch in a shared CQ-flags word plus `Ring::cq_overflow()`/`cq_overflow_count()` readers. The review of (7) also surfaced and fixed a pre-existing **fd-leak DoS** in the `recvmsg` SCM_RIGHTS writeback (both the new ring path and the reference `recvmsg` syscall path now close installed fds if the user-copy of the control message faults). No SlopRing known-limitations remain open.
...
> **Status**: ✅ complete — closed as part of the continuous `develop` history; the milestone is tagged `framekernel-phase-3` (no separate `framekernel-phase-2` tag, 2K.6).
...
- [x] **2K.6** Subsumed — phases 2 and 3 landed continuously on `develop` with no clean phase-2 boundary commit, so no separate `framekernel-phase-2` tag was cut; the combined Phase-2+3 milestone is marked by `framekernel-phase-3`.
...
- [x] **3F.2** Each opcode justified against an existing sync syscall path in the design notes: `OP_READ`→`file_read_fd`, `OP_WRITE`→`file_write_fd`, `OP_RECVMSG`→`unix_recvmsg`/`socket_recvfrom`, `OP_SEND`→`unix_send`/`socket_send`, `OP_ACCEPT`→`unix_accept`/`socket_accept`, `OP_POLL_ADD`→`file_poll_register_fd` (+ `file_poll_fused` to read readiness), `OP_TIMEOUT`→`wait_event_timeout` as the *harvest-wait deadline* (not an independently-firing op), `OP_CANCEL`→in-flight-table walk, `OP_NOP`→none. **No new blocking primitives** — each opcode runs the existing path's non-blocking *probe* (the path reads a *stored* nonblocking flag, not a per-call arg) and, on EAGAIN, records an in-flight row that the harvest phase re-probes; R12 parity holds because it is the *same code path*. AF_INET (raw user ptrs) vs AF_UNIX (kernel slices) marshalling differs per family.
- [x] **3F.3** Threat model documented in the design notes: SQEs are hostile user-controlled bytes in concurrently-mutable shared memory; snapshot-then-validate closes TOCTOU; every user VA re-validated through `UserSlice::try_new`; ring memory is `UFrame<RingMeta>` so Inv. 4 + Inv. 5 hold by construction (no kernel `&T` over it); stale/forged ring fds resolve via the Phase-2H `HandleTable` to typed `-EBADF`, never UB; and the core claim — **no kernel reference into the ring outlives a single `ring_enter` invocation** — is argued from the volatile byte-copy-only access discipline. The review also surfaced the access-level data race under the old non-atomic byte interface, the CQ-overflow fd-leak, the `sq_entries`-clamp DoS, and the mmap-after-close UAF.
...
#### Edge-case hardening (all resolved)

The follow-up pass closed every open SlopRing gap; for the record, what was fixed:

- **`OP_WRITE` / `OP_SEND` deferred send-readiness wakeup.** The socket `poll_fused`/`poll_unwait` now enqueue/dequeue the TX-drained send event (`sock_send_ev`) when `POLLOUT` is requested, so a deferred write that hit `-EAGAIN` on a full TX buffer is woken exactly when the buffer drains (not just by the harvest re-poll cap). Also benefits `poll(2)`/`ppoll` POLLOUT on sockets.
- **`OP_SEND` / `OP_RECVMSG` socket semantics.** `OP_SEND` routes to `socket_send`/`unix_send` (socket-typed, per-family, staged through a kernel buffer); `OP_RECVMSG` parses the user `MsgHdr` and routes to `socket_recvfrom`/`unix_recvmsg` with full SCM_RIGHTS fd-passing — no longer aliased to the generic `read`/`write` path.
- **CQ overflow visibility.** A sticky `SLOPRING_CQ_OVERFLOW` latch (in a shared CQ-flags word reusing existing padding) is set on a full-CQ drop; `Ring::cq_overflow()` / `cq_overflow_count()` expose it.
- **Pre-existing `recvmsg` SCM_RIGHTS fd-leak (DoS).** Surfaced while implementing `OP_RECVMSG`: the SCM_RIGHTS writeback installed fds before the user-copy of the control message, leaking them on a copy fault. Both the new ring path and the reference `recvmsg` syscall path now roll back (close) installed fds on any copy failure.
...
- [x] **3I.5** Tagged `framekernel-phase-3` at this close commit (landed directly on `develop`; no separate PR for this pre-alpha milestone).
- [x] **3I.6** Status → `complete` — the three-phase framekernel plan is fully delivered. There is no Phase 4 in this plan; future directions (ARM64/RISC-V, CHERI/MTE, a capability system, Linux-ABI breadth) are out of scope and listed in § 8 as candidates for a separate future plan.
