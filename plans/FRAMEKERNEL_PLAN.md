...
- [ ] **C.10** **Intentionally skipped** (mirrors B.9). The plan as written calls for LMbench-equivalent parity within ± 5 % of a pre-Phase-1 baseline, but **no such baseline was ever recorded** — § C.10 was added retroactively, and the synthetic LMbench-equivalent runner was originally scheduled for Phase 2 § 2J. Phase 2 § 2J was subsequently **dropped** (no meaningful baseline existed to gate against), so no such runner was built; the cross-phase ±5 %/±10 % budget in § 4.4 remains the standing aspiration if a perf-regression suite is wanted later.
...
| 2A.6 hot-path perf | `just test` slowest-test column for `test_page_alloc_*` and `test_heap_*` | No page-allocator or kernel-heap tests appear in the `>= 500 ms` slow-test list, so every page-alloc and heap test completes in < 500 ms after the refactor. (A dedicated LMbench-equivalent gate was planned for § 2J, since dropped.) |
...
- [x] **2H.1** Define `slopos-ostd/src/handle.rs`:
  ```rust
  pub struct Handle<T> {
      slot: u32,
      generation: u64,
  ...
  ```
- [x] **2H.2** Convert FD table to `HandleTable<File>`. Today's manual generation tracking goes away.
- [x] **2H.3** Convert pipe table to `HandleTable<Pipe>`.
- [x] **2H.4** Convert page-table handle (already partially done in `VmSpace.generation` from 1D.9) to use this primitive.
- [x] **2H.5** Convert task table (today's `PROCESS_VMS[MAX_PROCESSES]` slot scheme) to `HandleTable<Task>`. The `is_running` field stays for Inv. 8.
- [x] **2H.6** Verify: stale-FD test (close FD, reuse slot, attempt to read with old `Handle`) returns `HandleError::Stale`, not UB.

### Phase 2H § 2H — Outcomes

- **§2H.1's `Handle<T>` / `HandleTable<T>` primitive** landed as a new root module `slopos-ostd/src/handle.rs` (re-exported at the crate root, alongside `ring_buffer` / `atomic_bitmap`). `Handle<T> { slot: u32, generation: u64, _marker: PhantomData<fn() -> T> }` — the `fn() -> T` marker (not `PhantomData<T>`) is load-bearing: it makes `Handle<T>` unconditionally `Copy`/`Send`/`Sync` regardless of `T`, so a handle can live inside a `#[derive(Copy)]` `FdEntry` and reference a table whose value type is non-`Copy` (e.g. `KBox<Task>`). `HandleError` gained a `Full` variant (fixed-capacity exhaustion) beyond the planned `Stale` / `OutOfBounds` / `NoEntry`. `HandleTable<T>` is `KVec`-backed with `new()` (const, growable) / `with_capacity` / `with_fixed_capacity` (pre-filled, non-reallocating spine for lock-free scans) constructors, plus `insert` / `get` / `get_mut` / `remove` / `iter` / `handles` / `high_water`. **The entire primitive is pure safe Rust — it adds zero `unsafe` lines** (the lock-free read path reuses OSTD's existing `SpinLock::read_atomic_field`, which already folds the one `unsafe` reborrow). 10 host unit tests (incl. `stale_handle_after_reuse`, `fixed_capacity_recycles_without_growth`) run UB-free under `just check-miri`.

- **§2H.2's `HandleTable<File>`** retargeted the *global open-file table* (the table that actually carried the dead `generation: u16`), not the per-process FD-number array (FDs stay POSIX small-ints chosen lowest-first — that is the ABI and cannot become opaque handles). `OpenFileEntry` → `OpenFile` (the `generation: u16` and `valid: bool` fields deleted — the table owns liveness + generation); `OpenFilesState.open_files: [OpenFileEntry; 256]` → `HandleTable<OpenFile>`; the per-process `FdEntry.open_file_idx: u16` → `open_file: Handle<OpenFile>`. This **closes the real stale-`open_file_idx` use-after-reuse gap**: `snapshot_fd` now resolves through `HandleTable::get`, so a recycled open-file slot yields `EBADF` instead of aliasing the new occupant. Refcounting stays inside `OpenFile`; the FD layer calls `remove` on the last release. The poll subsystem's cross-layer token (`FusedPollResult.open_file_idx: u32`) widened to `open_file_token: u64` (a packed `Handle<OpenFile>`) so the fused→unfused cleanup path is generation-checked too; `PollRegInfo` stores the `Handle<OpenFile>` directly.

- **§2H.3's `HandleTable<Pipe>`** replaced `[SpinLock<PipeSlot>; 64]` + the `PIPE_ALLOC` bitmap + the `PIPE_GENERATION` atomic with one `SpinLock<HandleTable<Pipe>>`. `PipeSlot` → `Pipe` with the 4 KiB ring moved from inline BSS to a heap `KVec<u8>` (256 KiB of static BSS retired). `PipeHandle` survives as a `repr(transparent)` **packed `u64`** token (low 8 bits slot, top 56 bits generation) because the FD layer stores a pipe reference inside `OpenFile.handle: usize`, a width shared by every file backend — a 16-byte `Handle<Pipe>` cannot ride there, so the packing stays private to `pipe.rs` rather than polluting the generic `Handle`. The `lock_slot` guard pattern became `with_pipe` / `with_pipe_mut` closures that never let a table guard escape, preserving the documented release-before-wake ordering (the AB-BA defence in `pipe_file_ops.rs`). Per-slot locks collapse to one table lock — pipe ops are short ring memcpys that release before blocking, MAX_PIPES is 64, so contention is negligible (same single-lock tradeoff 2G took for the packet pool).

- **§2H.4's page-table handle** is delivered by §2H.5 rather than a standalone change: a process's `VmSpace` lives inside its VM slot, so a pid-keyed page-table reference is now generation-checked through `Handle<ProcessVm>` (stale slot → typed error). The pre-existing `VmSpace::generation` (1D.9) is a **mutation** counter (bumped per `CursorMut` commit), semantically orthogonal to slot reuse, and stays as the in-place "has this address space changed" signal. `VmSpace` itself is `KArc`-owned, so a held strong reference was already UB-free by refcount.

- **§2H.5 was split because the plan text conflated two distinct tables.** "Today's `PROCESS_VMS[MAX_PROCESSES]` slot scheme" is the **process-VM table** in `mm/src/process_vm.rs`; the "`is_running` field" note refers to the **scheduler task pool** in `sched/`. Both were converted, but **not by moving their storage into a heap `HandleTable`** — their pinned, per-slot-locked / lock-free-scanned storage is *superior* to heap move-in/move-out storage for their access patterns (the process table's lock-free pid scan feeds context-switch `activate` and user-copy `get_page_dir`; the scheduler pool hands raw `*mut Task` to assembly context-switch routines and must never reallocate). Folding either into a single-locked `HandleTable` would have *regressed* those hot paths into global serialisation — the opposite of the mandate. Instead the shared `Handle<T>` generation primitive was adopted as a **typed slot-reuse identity overlay** on the existing storage:
  - **Process table**: `ProcessVmInner` → `ProcessVm` (now a `pub` opaque marker so other crates can name `Handle<ProcessVm>`) with a `generation: u64` field stamped from a monotonic `VmSlotAlloc::next_generation` at every `create_process_vm` / `process_vm_clone_cow` bind. New `process_vm_handle(pid) -> Option<Handle<ProcessVm>>` + `process_vm_with_handle(handle, f) -> Result<_, HandleError>` give generation-checked, slot-reuse-safe access; the lock-free pid scan and per-slot `SpinLock<ProcessVm>` concurrency are untouched.
  - **Scheduler pool**: `TaskInner` (OSTD) gained a `generation: u64` tail field (after the asm-accessed fields, so the `offset_of!(abi) == 0` razor is unaffected; zero-initialised by the existing `write_bytes` in `init_invalid`). `reserve_task_slot` stamps a fresh generation (from `TaskManagerInner::next_generation`) on every slot reservation alongside `slot_index` / `task_id`. New `task_handle(task_id) -> Option<Handle<Task>>` + lock-free `task_resolve_handle(handle) -> *mut Task` (generation-checked, same pinned-spine invariants as `task_find_by_id`). `task_id` stays the human/POSIX-facing id, `is_running` / `on_cpu` stay for Inv. 8, and the assembly hot path keeps its raw `*mut Task`.

- **§2H.6** is proven at three levels: the `handle.rs` primitive test (`stale_handle_after_reuse`), a process-table KTAP test (`test_process_vm_handle_stale_after_reuse` — destroy then recreate on a reused slot ⇒ old handle resolves `Stale`/`NoEntry`, never the new occupant), and a scheduler KTAP test (`test_task_handle_stale_after_reuse` — the stale handle never resolves to a *different* task, and resolves to null once the slot is reclaimed with a fresh generation). The FD and pipe staleness is the `HandleTable` mechanism itself (proven by the primitive test) exercised by the full FD/pipe suites.

**Verification.**

| Gate | Recipe | Result |
|---|---|---|
| 2H.1 primitive | `cargo test -p slopos-ostd --lib handle::` | **10 passed, 0 failed** — incl. `stale_handle_after_reuse`, `fixed_capacity_recycles_without_growth`, `iter_yields_live_only`. |
| 2H framekernel gates | `just check-framekernel` | **All gates green**: `check_unsafe_outside_ostd: OK` (the primitive is pure safe Rust — no new `unsafe`), `check_alloc_dep: OK`, `check_stack_sizes: OK — all frames <= 2048 bytes` (the pipe ring moved to heap, so no 4 KiB stack rvalue), `check_wait_predicate_purity: OK`, `cargo fmt --all -- --check` clean, `just check-miri` 10 host tests + 10 doctests passed UB-free. |
| 2H functional parity | `just test` | **2457 tests across 2 phases → 2457 passed, 0 failed, 0 skipped, 1 over-time** (+3 vs the 2G baseline: the two new stale-handle KTAP tests plus `test_tickless_idle_past_deadline_no_overflow`). The lone over-time is the pre-existing `test_virtio_net_scan_discovers_network_members` at 524 ms. |
| 2H GUI smoke-test fix | `just boot-fast` | Smoke-testing the GUI surfaced a **pre-existing latent overflow** that 2H's runtime-timing shift made reachable: `scheduler::arm_tickless_idle_if_due` fed an already-due sleep deadline (`deadline <= now`, whose `wrapping_sub` lands near `u64::MAX`) into `ticks_to_ms_ceil`, where `saturating_mul(1000) + (freq - 1)` overflowed. Fixed at the source — the idle path now treats a past deadline as already-due and skips arming a one-shot (the periodic tick wakes it), and `ticks_to_ms_ceil` uses `saturating_add`. Regression test `test_tickless_idle_past_deadline_no_overflow` reproduces the exact path. |
| 2H count regression | `just check-test-count` | **OK** — total rose by the two new tests. |
| TCB ratio | `just tcb-ratio` | **901 / 124,792 = 0.722 %** (target Phase 2 ≤ 1.0 %; **down** from the 2G-close 0.724 % — Phase 2H added **zero** `unsafe` lines while growing total LoC, because `handle.rs` and every generation overlay are pure safe Rust). |

### 2I: Achieve `#![forbid(unsafe_code)]` on every non-OSTD kernel crate

The phase-2 closing audit.

- [x] **2I.1** `#![forbid(unsafe_code)]` is present on every non-OSTD kernel crate's `lib.rs`. The discipline was established crate-by-crate across Phase 1J and Phase 2A–2H, so this item lands as an audit: all 17 kernel crates carry it — `abi`, `acpi`, `boot`, `core`, `drivers`, `font`, `fs`, `gfx`, `hermetic`, `karch`, `kernel-services`, `mm`, `net`, `sched`, `service-core`, `video`, `windowing`. The lone binary crate `kernel/` carries `#![forbid(unsafe_op_in_unsafe_fn)]` instead (its `main.rs` is the documented global-allocator exempt site). The five `NO-FORBID` workspace members (`appkit`, `ktesting`, `slibc`, `slop-protocol`, `userland`) are userland-side and explicitly out of the discipline's scope per `CLAUDE.md`.
- [x] **2I.2** `scripts/check_unsafe_outside_ostd.sh` → `check_unsafe_outside_ostd: OK — kernel crates outside slopos-ostd contain no executable unsafe`. Zero matches.
- [x] **2I.3** `just tcb-ratio` → **901 / 124,792 = 0.722 %** (target ≤ 1.0 %).
- [x] **2I.4** `CLAUDE.md` already reflects post-Phase-2 reality (the "Unsafe-code surface" + "Allocation discipline" sections enumerate all kernel crates including `sched`/`gfx`, describe the four build gates, and document the two exempt sites). No leftover Phase-1-only prose remained; the only "Phase 1"/"Phase 2" mention is the `tcb_ratio.sh` informational target line (`Phase 1 ≤ 1.5 %; Phase 2 ≤ 1.0 %`), which is correct and intentionally retained.

### 2K: Phase 2 close

- [x] **2K.1** `just check-framekernel` — **all six sub-gates green**: `check_unsafe_outside_ostd: OK`; `check_alloc_dep: OK`; `check_stack_sizes: OK — all frames <= 2048 bytes`; `check_wait_predicate_purity: OK`; `cargo fmt --all -- --check` clean; `just check-miri` **407 passed / 29 ignored / 0 failed** across all OSTD test binaries. (The close also fixed a latent stack-frame fragility: `slopos_mm::slab::stats::snapshot` folded its eight per-size-class counters with eight inlined macro expansions whose debug-build temporaries weren't coalesced, giving a 2616 B frame whenever the optimizer left the function out-of-line — over the 2 KiB Inv. 5' ceiling. Rewritten to a single loop over an `[&SlabClassStats; 8]` array, so the frame is small and stable regardless of inlining.)
- [x] **2K.2** `just test` — **2458 tests across 2 phases → 2458 passed, 0 failed, 0 skipped, 1 over-time** (kernel 2453 + userland 5). The +1 vs. the 2H-close 2457 is the new `test_ext2_zero_inodes_per_group_rejected` regression (see 2K.5). The lone over-time is the pre-existing `test_virtio_net_scan_discovers_network_members` (~524 ms vs the 500 ms warn threshold), unrelated to Phase 2.
- [x] **2K.3** TCB ratio **0.722 %** ≤ 1.0 %.
- [x] **2K.4** Performance verification was **dropped from Phase 2 by direction** — the original § 2J (synthetic LMbench-equivalent runner + baseline comparison) was removed, since no pre-Phase-1 baseline was ever recorded (see § C.10) to gate against without inventing numbers. The cross-phase ±5 %/±10 % budget in § 4.4 stands as the aspiration if a perf-regression suite is wanted later; the per-subphase "perf within ±5 %" checks (2A.6, 2B.5, 2C.8) stand as landed, each verified against the `just test` slow-test list at the time.
- [x] **2K.5** `CVSS.md` updated to the post-Phase-2 attack surface under the **pre-alpha "open-findings-only" policy** — resolved findings are removed from the ledger, not retained as historical `fixed` records (the lifecycle rule in `AGENTS.md`/`CLAUDE.md` was updated to match):
  - The UI/input **raw-user-pointer** findings (formerly 0001–0004) are resolved and removed. The `#![forbid(unsafe_code)]` discipline on `core/`/`video/`/`drivers/` makes a raw `*event_ptr = …` / `from_raw_parts(user_ptr, …)` a compile error, and § 2D rewrote those handlers onto typed `UserPtr` / `UserBytes` args with `copy_to_user` / `copy_from_user` — the class is architecturally eliminated, not merely patched.
  - The ext2 **superblock divide-by-zero** (formerly 0005) is resolved and removed. `Superblock::parse` (`fs/src/ext2/ondisk.rs`) now rejects `inodes_per_group == 0 || blocks_per_group == 0 || inodes_count == 0` before any divisor is used; regression test `test_ext2_zero_inodes_per_group_rejected`.
  - **SLOPOS-2026-0006** (ext2 OOB slicing on untrusted `inode_size`/offsets) remains the **sole open finding** — pre-existing, not a Phase 2 deliverable; `fs/` is `#![forbid(unsafe_code)]`, so the worst case is a bounded-slice **panic** / DoS, never UB.
- [ ] **2K.6** _Pending — user-performed._ All close gates are green and the working tree carries the close changes (plan + `CVSS.md` + the ext2 superblock geometry guard/test + the `slab::stats::snapshot` stack-frame fix); the remaining step is to commit them and tag `framekernel-phase-2` at that SHA, mirroring the Phase-1 `framekernel-phase-1` precedent on `develop`.
- [x] **2K.7** Front-matter status flipped `phase-2-ready` → `phase-3-ready`.
...
4. Generation-counter handles in place for fds, pipes, page tables, tasks. ✅
5. `CVSS.md` reflects post-Phase-2 attack surface. ✅

(The original Exit Criterion on macro-benchmark performance was retired together with the dropped § 2J performance-verification work; see 2K.4.)
