...
**Status: in progress.** η.1, η.2, η.3, η.5, η.6 landed. η.4 (legacy paging deletion + kernel-side helper migration) pending — the framekernel boundary is established (hardware CR3 = OSTD `VmSpace::pml4_paddr`, all fault paths route through OSTD), so the residual legacy code is dead-write deadweight rather than a load-bearing dependency. 2409 kernel + 215 OSTD tests passing throughout. Detailed status:
