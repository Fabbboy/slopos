...
//!   the PCID's existing TLB entries across the load.
...
/// 12-bit hardware process-context identifier: the CPU tags TLB entries with
/// the PCID in CR3[11:0] so entries from different address spaces can coexist.
/// A `Pcid` is always bound to a particular CPU's slot pool (see
/// `mm::mmu::asid`), so a raw value alone cannot reconstruct a CR3 — the owning
/// `MmContext` generation is needed too.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
#[repr(transparent)]
pub struct Pcid(u16);

impl Pcid {
    /// Kernel / bootstrap PCID. Kernel mappings use this whenever PCID is
    /// enabled, and are also marked `GLOBAL` so they outlive CR3 reloads.
    pub const KERNEL: Pcid = Pcid(0);

    /// `None` when `raw` does not fit the architectural 12-bit field.
    #[inline]
    pub const fn new(raw: u16) -> Option<Self> {
        if raw <= 0x0FFF { Some(Self(raw)) } else { None }
    }

    /// Caller guarantees `raw < 4096`.
    #[inline]
...
/// A stable, monotonically-increasing identifier for a single `MmContext`
/// (i.e. a process address space). Unlike `Pcid` it is **never reused**: the
/// hardware PCID is a separate resource, so a context that migrates between
/// CPUs or is evicted from a PCID slot keeps its `MmContextId` and is assigned
/// a fresh `Pcid` by the per-CPU ASID pool.
#[derive(Clone, Copy, Debug, PartialEq, Eq, PartialOrd, Ord, Hash)]
#[repr(transparent)]
pub struct MmContextId(u64);

impl MmContextId {
    /// Sentinel for "no context": an empty per-CPU ASID slot, or an unloaded
    /// address space in a page-fault report.
    ...
    /// For a context ID derived from the legacy `process_id` surface rather
    /// than freshly allocated by `alloc_mm_context_id`.
    #[inline]
...
/// A complete `CR3` value: 52-bit PML4 physical frame, 12-bit PCID, optional
/// no-flush bit. `Debug` shows the decomposed fields, so tracing a stale TLB
/// bug does not mean decoding a raw hex value by hand.
#[derive(Clone, Copy, PartialEq, Eq)]
#[repr(transparent)]
pub struct Cr3Value(u64);

impl Cr3Value {
    /// Isolates the 52-bit physical frame; matches `PageFlags::ADDRESS_MASK`.
    ...
    /// Build a CR3 value from a page-aligned PML4 physical address, a PCID and
    /// a no-flush selector. Panics in debug builds on an unaligned `pml4_phys`.
    #[inline]
    ...
    /// Reinterpret a raw CR3 read from hardware or assembly.
    #[inline]
...
/// The single choke point for every CR3 write in the kernel; raw `u64`s never
