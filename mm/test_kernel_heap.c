...
int test_heap_free_list_search(void) {
    klog_printf(KLOG_INFO, "HEAP_TEST: Starting free-list search regression test\n");
    ...
    klog_printf(KLOG_INFO, "HEAP_TEST: Initial heap size: %lu bytes\n", initial_heap_size);

    /* Step 1: Allocate a small block that will be at the head */
    void *small_ptr = kmalloc(32);
    if (!small_ptr) {
        klog_printf(KLOG_INFO, "HEAP_TEST: Failed to allocate small block\n");
        return -1;
    }
    klog_printf(KLOG_INFO, "HEAP_TEST: Allocated small block at head (32 bytes)\n");

    /* Step 2: Allocate a larger block (this will be in a larger size class or later) */
    void *large_ptr = kmalloc(1024);
    if (!large_ptr) {
        klog_printf(KLOG_INFO, "HEAP_TEST: Failed to allocate large block\n");
        kfree(small_ptr);
        return -1;
    }
    klog_printf(KLOG_INFO, "HEAP_TEST: Allocated large block (1024 bytes)\n");

    /* Step 3: Allocate another medium block to create fragmentation */
    void *medium_ptr = kmalloc(256);
    if (!medium_ptr) {
        klog_printf(KLOG_INFO, "HEAP_TEST: Failed to allocate medium block\n");
        kfree(small_ptr);
        kfree(large_ptr);
        return -1;
    }
    klog_printf(KLOG_INFO, "HEAP_TEST: Allocated medium block (256 bytes)\n");
     ...
     */
    kfree(large_ptr);
    klog_printf(KLOG_INFO, "HEAP_TEST: Freed large block\n");
    
    kfree(small_ptr);
    klog_printf(KLOG_INFO, "HEAP_TEST: Freed small block\n");
    ...
    if (!requested_size) {
        klog_printf(KLOG_INFO, "HEAP_TEST: Failed to allocate 512-byte block (should have found free space)\n");
        kfree(medium_ptr);
        get_heap_stats(&stats_after);
        
        /* Check if heap expanded unnecessarily */
        if (stats_after.total_size > mid_heap_size) {
            klog_printf(KLOG_INFO, "HEAP_TEST: FAILED - Heap expanded despite having suitable free block\n");
            return -1;
        }
        return -1;
    }
    klog_printf(KLOG_INFO, "HEAP_TEST: Successfully allocated 512-byte block\n");
    ...
    if (final_heap_size > mid_heap_size) {
        klog_printf(KLOG_INFO,
                    "HEAP_TEST: FAILED - Heap expanded from %lu to %lu bytes despite having sufficient free space\n",
                    mid_heap_size, final_heap_size);
        
        klog_printf(KLOG_INFO, "HEAP_TEST: Free size before allocation: %lu bytes\n", stats_mid.free_size);
    ...
    klog_printf(KLOG_INFO, "HEAP_TEST: Heap did not expand (correct behavior)\n");
    klog_printf(KLOG_INFO, "HEAP_TEST: Heap size remained at %lu bytes\n", final_heap_size);
    ...
    if (cleanup_heap_size != final_heap_size) {
        klog_printf(KLOG_INFO, "HEAP_TEST: WARNING - Heap size changed during cleanup\n");
    }

    klog_printf(KLOG_INFO, "HEAP_TEST: Free-list search regression test PASSED\n");
    return 0;
}
...
int test_heap_fragmentation_behind_head(void) {
    klog_printf(KLOG_INFO, "HEAP_TEST: Starting fragmentation behind head test\n");
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (!ptrs[i]) {
            klog_printf(KLOG_INFO, "HEAP_TEST: Failed to allocate block %d\n", i);
    ...
    klog_printf(KLOG_INFO, "HEAP_TEST: Allocated 5 blocks\n");
    ...
    kfree(ptrs[0]);
    klog_printf(KLOG_INFO, "HEAP_TEST: Freed block 0 (small, now at head)\n");

    /* Free index 2 (another small, might coalesce or stay separate) */
    kfree(ptrs[2]);
    klog_printf(KLOG_INFO, "HEAP_TEST: Freed block 2 (small)\n");

    /* Now free a larger one (index 1 or 3) */
    kfree(ptrs[3]); /* 512 bytes - larger */
    klog_printf(KLOG_INFO, "HEAP_TEST: Freed block 3 (large, should be behind head in list)\n");
    ...
    if (!needed) {
        klog_printf(KLOG_INFO, "HEAP_TEST: Failed to allocate 400-byte block\n");
        kfree(ptrs[1]);
        kfree(ptrs[4]);
        get_heap_stats(&stats_after);
        if (stats_after.total_size > allocated_heap_size) {
            klog_printf(KLOG_INFO, "HEAP_TEST: FAILED - Heap expanded when suitable block exists\n");
            return -1;
        }
        return -1;
    }
    ...
    if (final_heap_size > allocated_heap_size) {
        klog_printf(KLOG_INFO,
                    "HEAP_TEST: FAILED - Heap expanded from %lu to %lu bytes\n",
                    allocated_heap_size, final_heap_size);
        klog_printf(KLOG_INFO, "HEAP_TEST: This indicates the free-list search missed a suitable block\n");
    ...
    klog_printf(KLOG_INFO, "HEAP_TEST: Successfully allocated without heap expansion\n");
    klog_printf(KLOG_INFO, "HEAP_TEST: Heap size: %lu bytes (no change)\n", allocated_heap_size);
    ...
    klog_printf(KLOG_INFO, "HEAP_TEST: Fragmentation behind head test PASSED\n");
    return 0;
}
...
int run_kernel_heap_tests(void) {
    klog_printf(KLOG_INFO, "HEAP_TEST: Running kernel heap regression tests\n");
    ...
    if (test_heap_free_list_search() == 0) {
        ...
        klog_printf(KLOG_INFO, "HEAP_TEST: test_heap_free_list_search FAILED\n");
    }
    ...
    if (test_heap_fragmentation_behind_head() == 0) {
        ...
        klog_printf(KLOG_INFO, "HEAP_TEST: test_heap_fragmentation_behind_head FAILED\n");
    }

    klog_printf(KLOG_INFO, "HEAP_TEST: Completed %d tests, %d passed\n", total, passed);
