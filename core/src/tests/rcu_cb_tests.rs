...
//!
//! `call_rcu` only queues, so the reachability test never calls the drain by hand.
...
/// Drives the drain itself rather than `rcu_barrier`, which a stalled grace period never leaves.
pub fn drain_until(done: impl Fn() -> bool) -> bool {
    ...
    loop {
        ...
        // No `rcu_note_qs` of its own: a loop that reports would spend the budget below itself.
...
/// still pass every correctness test, so the passes are counted at the wait itself.
pub fn test_rcu_drain_never_waits_for_a_grace_period() -> TestResult {
    ...
    // Sampled inside one masked window with the reads it indexes: a `cpu` taken before the
    // window can name a slot the reports never touched.
