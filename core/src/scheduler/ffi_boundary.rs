...
//!   - `scheduler_task_exit`: callable from Rust (`kthread`); also
//!     reachable via `task_entry_trampoline` in
//!     `super::switch_asm`, which uses `sym
//!     scheduler_task_exit_impl` directly so that path doesn't go
//!     through this module.
...
//!
//! `super::switch_asm` (Rust naked functions) is the sole context-
//! switch implementation; the historical extern-C helpers that used
//! to live in a separate asm file (`context_switch`,
//! `context_switch_user`, `simple_context_switch`,
//! `init_kernel_context`, `task_entry_wrapper`,
//! `context_switch_bad_target`) are gone.

/// Task exit hook called from `super::switch_asm::task_entry_trampoline`
