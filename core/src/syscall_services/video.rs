...
slopos_lib::define_service! {
    video => VideoServices {
        ...
        surface_set_window_position(task_id: u32, x: i32, y: i32) -> c_int;
        surface_set_window_state(task_id: u32, state: u8) -> c_int;
        surface_raise_window(task_id: u32) -> c_int;
        surface_commit(task_id: u32) -> c_int;
        register_surface(task_id: u32, width: u32, height: u32, shm_token: u32) -> c_int;
        drain_queue();
        surface_request_frame_callback(task_id: u32) -> c_int;
        surface_mark_frames_done(present_time_ms: u64);
        surface_poll_frame_done(task_id: u32) -> u64;
        surface_add_damage(task_id: u32, x: i32, y: i32, width: i32, height: i32) -> c_int;
        surface_get_buffer_age(task_id: u32) -> u8;
        surface_set_role(task_id: u32, role: u8) -> c_int;
        surface_set_parent(task_id: u32, parent_task_id: u32) -> c_int;
        surface_set_relative_position(task_id: u32, rel_x: i32, rel_y: i32) -> c_int;
        @no_wrapper fb_flip(phys_addr: PhysAddr, size: usize) -> c_int;
        @no_wrapper roulette_draw(fate: u32) -> c_int;
        @no_wrapper surface_set_title(task_id: u32, ptr: *const u8, len: usize) -> c_int;
    }
}
