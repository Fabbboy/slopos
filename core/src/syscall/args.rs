...
// ─────────────────────────────────────────────────────────────────────
// Compositional `SyscallArgList` (hand-rolled per arity 0..=6)
//
// The `define_syscall!` macro expands per-parameter directly into
// `<T as SyscallArg>::from_raw(...)` calls — the list type is exposed
// here so callers can also choose tuple-shaped parsing if they want.
// ─────────────────────────────────────────────────────────────────────

pub trait SyscallArgList: Sized {
    fn parse(ctx: &SyscallContext) -> Result<Self, Errno>;
}

impl SyscallArgList for () {
    #[inline]
    fn parse(_ctx: &SyscallContext) -> Result<Self, Errno> {
        Ok(())
    }
}
...
impl<A: SyscallArg> SyscallArgList for (A,) {
    fn parse(ctx: &SyscallContext) -> Result<Self, Errno> {
        const {
            assert!(<A as SyscallArg>::ARITY <= 6, "syscall args overflow regs");
        }
        let regs = ctx.regs();
        Ok((A::from_raw(&regs[0..A::ARITY], ctx)?,))
    }
}
...
impl<A: SyscallArg, B: SyscallArg> SyscallArgList for (A, B) {
    fn parse(ctx: &SyscallContext) -> Result<Self, Errno> {
        const {
            assert!(<A as SyscallArg>::ARITY + <B as SyscallArg>::ARITY <= 6);
        }
        let regs = ctx.regs();
        let c1 = <A as SyscallArg>::ARITY;
        let c2 = c1 + <B as SyscallArg>::ARITY;
        let a = A::from_raw(&regs[0..c1], ctx)?;
        let b = B::from_raw(&regs[c1..c2], ctx)?;
        Ok((a, b))
    }
}

impl<A: SyscallArg, B: SyscallArg, C: SyscallArg> SyscallArgList for (A, B, C) {
    fn parse(ctx: &SyscallContext) -> Result<Self, Errno> {
        const {
            assert!(
                <A as SyscallArg>::ARITY + <B as SyscallArg>::ARITY + <C as SyscallArg>::ARITY <= 6
            );
        }
        let regs = ctx.regs();
        let c1 = <A as SyscallArg>::ARITY;
        let c2 = c1 + <B as SyscallArg>::ARITY;
        let c3 = c2 + <C as SyscallArg>::ARITY;
        let a = A::from_raw(&regs[0..c1], ctx)?;
        let b = B::from_raw(&regs[c1..c2], ctx)?;
        let c = C::from_raw(&regs[c2..c3], ctx)?;
        Ok((a, b, c))
    }
}

impl<A: SyscallArg, B: SyscallArg, C: SyscallArg, D: SyscallArg> SyscallArgList for (A, B, C, D) {
    fn parse(ctx: &SyscallContext) -> Result<Self, Errno> {
        const {
            assert!(
                <A as SyscallArg>::ARITY
                    + <B as SyscallArg>::ARITY
                    + <C as SyscallArg>::ARITY
                    + <D as SyscallArg>::ARITY
                    <= 6
            );
        }
        let regs = ctx.regs();
        ...
        let c2 = c1 + <B as SyscallArg>::ARITY;
        let c3 = c2 + <C as SyscallArg>::ARITY;
        let c4 = c3 + <D as SyscallArg>::ARITY;
        let a = A::from_raw(&regs[0..c1], ctx)?;
        let b = B::from_raw(&regs[c1..c2], ctx)?;
        let c = C::from_raw(&regs[c2..c3], ctx)?;
        let d = D::from_raw(&regs[c3..c4], ctx)?;
        Ok((a, b, c, d))
    }
}

impl<A: SyscallArg, B: SyscallArg, C: SyscallArg, D: SyscallArg, E: SyscallArg> SyscallArgList
    for (A, B, C, D, E)
{
    fn parse(ctx: &SyscallContext) -> Result<Self, Errno> {
        const {
        ...
        let c3 = c2 + <C as SyscallArg>::ARITY;
        let c4 = c3 + <D as SyscallArg>::ARITY;
        let c5 = c4 + <E as SyscallArg>::ARITY;
        let a = A::from_raw(&regs[0..c1], ctx)?;
        let b = B::from_raw(&regs[c1..c2], ctx)?;
        let c = C::from_raw(&regs[c2..c3], ctx)?;
        let d = D::from_raw(&regs[c3..c4], ctx)?;
        let e = E::from_raw(&regs[c4..c5], ctx)?;
        Ok((a, b, c, d, e))
    }
}

impl<A: SyscallArg, B: SyscallArg, C: SyscallArg, D: SyscallArg, E: SyscallArg, F: SyscallArg>
    SyscallArgList for (A, B, C, D, E, F)
{
    fn parse(ctx: &SyscallContext) -> Result<Self, Errno> {
        const {
            assert!(
        ...
        let c4 = c3 + <D as SyscallArg>::ARITY;
        let c5 = c4 + <E as SyscallArg>::ARITY;
        let c6 = c5 + <F as SyscallArg>::ARITY;
        let a = A::from_raw(&regs[0..c1], ctx)?;
        let b = B::from_raw(&regs[c1..c2], ctx)?;
        let c = C::from_raw(&regs[c2..c3], ctx)?;
        let d = D::from_raw(&regs[c3..c4], ctx)?;
        let e = E::from_raw(&regs[c4..c5], ctx)?;
        let f = F::from_raw(&regs[c5..c6], ctx)?;
        Ok((a, b, c, d, e, f))
    }
}
