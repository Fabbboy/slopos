...
impl<O: gimli::ReaderOffset> gimli::UnwindContextStorage<O> for StoreInVec {
    type Rules = Vec<(Register, RegisterRule<O>)>;
    type Stack = Vec<UnwindTableRow<O, Self>>;
}

#[cfg(feature = "dwarf-expr")]
impl<R: gimli::Reader> gimli::EvaluationStorage<R> for StoreInVec {
    type Stack = Vec<Value>;
    type ExpressionStack = Vec<(R, R)>;
    type Result = Vec<gimli::Piece<R>>;
}

#[derive(Debug)]
pub struct Frame {
    fde_result: FDESearchResult,
    row: UnwindTableRow<usize, StoreInVec>,
}
...
impl Frame {
    pub fn from_context(ctx: &Context, signal: bool) -> Result<Option<Self>, gimli::Error> {
        ...
        let mut unwinder = UnwindContext::<_, StoreInVec>::new_in();
        let row = fde_result
            .fde
    ...
    fn evaluate_expression(
        ...
        let mut eval =
            Evaluation::<_, StoreInVec>::new_in(expr.0, self.fde_result.fde.cie().encoding());
