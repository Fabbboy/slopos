...
- `just test-persist` — two boots of one image with no rebuild between: write + `fsync`, power off, read back. In CI after `check-fs-image`. Needs its own boots and cannot reuse the shared capture.
