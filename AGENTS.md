...
- **`scripts/check_safe_contract_surface.sh`** — ratchet on safe `pub fn`s in `slopos-ostd/` that carry a `# Safety` section. Those are self-declared caller obligations the compiler does not check; the baseline (currently 16) may fall but never rise. Not a count of safe fns containing `unsafe` — that is the design working, not a defect.
