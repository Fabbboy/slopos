...
// Firmware framebuffer the target's GOP leaves live: PLANE_SURF (DSPASURF) at
// GGTT byte 0x0112c000, a 1920x1080 XRGB8888 surface rounded up to 8 MiB here.
...
/// The target laptop's iGPU (8086:a7a8) is an Alder/Raptor-Lake-P Gen13 display.
...
pub fn xe_platform_known_dids() -> TestResult {
    // Tiger Lake-U GT2 and Alder Lake-S.
    ...
    // Alder Lake-P GT2 and a Raptor-Lake-P A7xx sibling.
...
/// The live `DSPACNTR=0x94009000` captured from the target decodes to
/// enable | XRGB8888 | Y-tiled | render-decompressed | BGRX order.
pub fn xe_plane_decode_live_dspacntr() -> TestResult {
    ...
    // The live read-back snapshot: 1920x1080 at GGTT 0x0112c000, Y-tiled stride
    // unit 0x3c.
...
/// Only the 32-bit RGB codes map to a plane format; the 24-bit packed formats
...
pub fn xe_plane_pixel_format_mapping() -> TestResult {
    ...
    // The 0b0100 format field decodes back to Rgb8888.
    assert_eq_test!(
...
/// Linear `PLANE_STRIDE` is a count of 64-byte units.
...
/// Unknown tokens and malformed values are ignored, so a typo cannot stop the
/// driver; `xe.modeset=off` is the recovery escape.
...
/// On display IP version 13 the mode code is ORed with `MCURSOR_ARB_SLOTS(1)`
/// (Wa_22012358565); earlier versions get the bare mode code.
pub fn xe_cursor_ctl_value() -> TestResult {
    // MCURSOR_MODE_{64,128,256}_ARGB_AX = 0x27 / 0x22 / 0x23.
    assert_eq_test!(
    ...
    // The arbitration-slot field is [30:28], never overlapping the mode bits.
    assert_eq_test!(
        regs::reg_field_get(
    ...
    // The ARGB promote bit (bit 5) rides on every enabled mode.
    assert_test!(
        cursor_config::cur_ctl_value(cursor_config::CursorMode::Argb64, true, 13)
    ...
    // Disabling clears the whole register; the WA bit is never set on a
    // disabled cursor.
    assert_eq_test!(
...
pub fn xe_cursor_pos_pack() -> TestResult {
    ...
    // Negative (-5, -3): magnitudes in the fields, both sign bits set.
...
pub fn xe_regs_field_helpers() -> TestResult {
    ...
    // Format field [27:24]; 4 is the 8:8:8:8 code.
    ...
    // Tiling field [12:10]; 4 is Y-tiled.
...
/// The END field holds the inclusive last block (end-exclusive minus one); a
/// zero register is the empty range.
...
/// The cursor's blocks come off the tail of the primary's allocation.
