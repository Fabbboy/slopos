...
//!
//! Pure data and `const fn` bit math: no MMIO, no I/O.
//!
...
/// GGTT table size for a 16 MiB GTTMMADR — this target.
pub const GGTT_TABLE_SIZE_8MB: usize = 8 * 1024 * 1024;

/// GGTT table size for an 8 MiB GTTMMADR variant.
...
/// GGTT TLB so it observes the new translations.
...
/// The three display pipes; consecutive pipes' register blocks are
/// `PIPE_REG_STRIDE` apart.
#[repr(usize)]
...
/// Gen12 programs `PLANE_WM` levels 0..=7.
...
/// Watermark register for `level` (0..[`PLANE_WM_LEVELS`)).
...
/// Pipe configuration (a.k.a. TRANSCONF on Gen12).
...
/// Cursor watermark register for `level` (0..[`PLANE_WM_LEVELS`)).
...
// PLANE_BUF_CFG / CUR_BUF_CFG share one layout, as do PLANE_WM / CUR_WM.
...
/// The level is satisfied by the blocks field alone; the lines field is unused.
...
/// Wa_22012358565: left at 0, the cursor plane's memory-fetch arbitration
/// starves and corrupts the PRIMARY plane's fetch.
...
/// YUV range-correction disable; always set for RGB source pixels.
...
/// XRGB8888 and ARGB8888: alpha participation is selected separately
/// (alpha-mode / PLANE_COLOR_CTL) and channel order by
/// `PLANE_CTL_COLOR_ORDER_RGBX`.
...
/// Place `value` into `mask`'s field; bits that do not fit are masked off.
