...
//!
//! The transfer engine is **polled**; no controller interrupt is required.
...
// LPSS clock-divider register: bit31 = enable, [30:16] = N, [15:1] = M. The
// DesignWare APB registers are clocked by it; gated, `IC_COMP_TYPE` reads 0.
const LPSS_CLK_ENABLE: u32 = 1 << 31;
const LPSS_CLK_MN_MASK: u32 = 0x7fff_fffe;
/// 1:1 passthrough (M = N = 1).
const LPSS_CLK_MN_1_1: u32 = (1 << 1) | (1 << 16);

/// Default functional ("ic_clk") frequency in kHz; SCL counts derive from it.
...
pub enum I2cError {
    /// `IC_COMP_TYPE` did not read back the DesignWare magic.
    NotDesignWare,
...
/// Real-time bound for one status-poll wait: a wedged controller fails fast
/// instead of monopolising the CPU.
...
impl<M: Mmio32> DesignWareI2c<M> {
    /// Wrap a mapped controller window; call [`init`](Self::init) before transferring.
    ...
    /// Release the LPSS wrapper from reset and point its address-remap register
    /// at the controller's own physical base.
    pub fn lpss_bringup(&self, mmio_phys_base: u64) {
        ...
        // Leave firmware's M/N divider alone unless both fields are zero, in
        // which case force 1:1 so the divider doesn't stall.
    ...
    /// Verify the core, read FIFO depths, program master/fast-mode config and SCL
    /// timing. Leaves the controller disabled and interrupts masked.
    ...
    /// Check for and clear a transfer abort.
    ...
    /// Perform one transaction. Interleaves FIFO fill and drain, so transfers
    /// larger than the FIFO are handled.
        ...
        while cmds_left > 0 || reads_left > 0 {
            ...
            while rx_avail > 0 && reads_left > 0 {
                        ...
                        || rx_byte >= segs[rx_seg].len())
                {
                    ...
                    {
                        // TODO(tech-debt): empty branch — delete it.
                    }
                    rx_seg += 1;
                    rx_byte = 0;
                }
    ...
    /// Convenience: write `tx`, repeated-START, read into `rx`.
