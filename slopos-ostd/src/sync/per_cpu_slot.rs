...
//!
//! Callers index by CPU number rather than reaching through `CpuLocal`'s
//! current-CPU accessor: the LUF drain rings and the ASID slot table are both
//! walked by explicit index, including from a CPU other than the slot's owner.
//!
//! The [`IrqDisabled`] argument carries the interrupts-off obligation; the
//! no-aliasing one cannot be typed — two sequential calls in one scope produce
//! two `&mut` and the compiler has no reason to object — so an atomic borrow
//! word checks it in every build. Both hazards then become a declined borrow
//! rather than aliasing UB: an IPI handler re-entering a slot its own CPU is
//! mid-update on, and a foreign CPU naming that slot by index.
//!
//! Shared and exclusive borrows follow `RefCell`'s rule, with the release in a
//! drop guard, so an unwind out of the closure cannot strand the slot.
...
/// Writer-held sentinel, unreachable as a reader count because that count is
/// bounded by the number of CPUs.
