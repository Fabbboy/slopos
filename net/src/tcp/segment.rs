...
//!
//! The [`SegmentBuilder`] type provides factory methods for every outgoing
//! segment kind; state handlers should prefer it over inline literals.
...
pub struct TcpOutSegment {
    ...
    pub mss: Option<u16>,
    /// Window Scale option shift count.
...
pub fn write_tcp_segment(seg: &TcpOutSegment, payload: &[u8], out: &mut [u8]) -> Option<usize> {
    ...
    if has_ts {
        // NOP + NOP + TSopt (kind + len + TSval + TSecr) = 12 bytes
        opt_len += 12;
    }
    if sack_n > 0 {
